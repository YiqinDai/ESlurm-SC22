 ...
 *  slurm.h - Definitions for all of the Slurm RPCs
 This file is modified by Yiqin Dai.
 Contact us: daiyq@nudt.edu.cn
 *****************************************************************************
 ...
 */
#define SLURM_VERSION_NUMBER 0x140b07
...
#define JOB_REQUEUE_CRON  0x01000000 /* This is a cron job. */
...
typedef enum {
	READY_NONE = 0,
	READY_NODE_STATE = 1 << 0,	/* job's node's are ready */
	READY_JOB_STATE = 1 << 1,	/* job is ready to execute */
	READY_PROLOG_STATE = 1 << 2,	/* PrologSlurmctld is done */
} job_node_ready_state_t;
...
#define MAIL_INVALID_DEPEND 0x0400	/* Notify on job invalid dependency */
...
#define NODE_STATE_DYNAMIC       0x00080000 /* node is dynamically assigned */
#define NODE_STATE_REBOOT_ISSUED 0x00100000 /* node reboot passed to agent */
...
#define PRIORITY_FLAGS_ACCRUE_ALWAYS	0x00000001 /* Flag to always accrue age
						    ...
						    * holds */
#define PRIORITY_FLAGS_MAX_TRES 	0x00000002 /* Calculate billed_tres as
...
#define HET_JOB_FLAG       0x00002000	/* Heterogeneous job management flag */
#define JOB_NTASKS_SET     0x00004000	/* --ntasks explicitly set */
#define JOB_CPUS_SET       0x00008000	/* --cpus-per-tasks explicitly set */
#define BF_WHOLE_NODE_TEST 0x00010000	/* Backfill test in progress */
...
#define CRON_JOB           0x00400000   /* Job submitted through scrontab */
...
#define JOB_DEPENDENT      0x20000000	/* Job dependent or invalid depend */
#define JOB_MAGNETIC       0x40000000	/* Job attempting to run in a
					 * magnetic reservation */
#define JOB_PART_ASSIGNED  0x80000000   /* Job didn't request a partition */
...
/* Added by Yiqin Dai */
typedef struct mediate_node_info_msg {
	uint16_t normal_mediate_cnt;
	char* normal_mediate_nodelist;
	uint16_t wrong_mediate_cnt;
	char* wrong_mediate_nodelist;
	uint16_t busy_mediate_cnt;
	char* busy_mediate_nodelist;
} mediate_node_info_msg_t;
/* Added by Yiqin Dai (end) */
 ...
 */
#define RESERVE_FLAG_MAINT	   0x0000000000000001	/* Set MAINT flag */
#define RESERVE_FLAG_NO_MAINT	   0x0000000000000002	/* Clear MAINT flag */
#define RESERVE_FLAG_DAILY	   0x0000000000000004	/* Set DAILY flag */
#define RESERVE_FLAG_NO_DAILY	   0x0000000000000008	/* Clear DAILY flag */
#define RESERVE_FLAG_WEEKLY	   0x0000000000000010	/* Set WEEKLY flag */
#define RESERVE_FLAG_NO_WEEKLY	   0x0000000000000020	/* Clear WEEKLY flag */
#define RESERVE_FLAG_IGN_JOBS	   0x0000000000000040	/* Ignore running jobs */
#define RESERVE_FLAG_NO_IGN_JOB	   0x0000000000000080	/* Clear ignore running
							 * jobs flag */
#define RESERVE_FLAG_ANY_NODES	   0x0000000000000100	/* Use any compute
							 * nodes */
#define RESERVE_FLAG_NO_ANY_NODES  0x0000000000000200	/* Clear any compute
							 * node flag */
#define RESERVE_FLAG_STATIC        0x0000000000000400	/* Static node allocation */
#define RESERVE_FLAG_NO_STATIC     0x0000000000000800	/* Clear static node
							 * allocation */
#define RESERVE_FLAG_PART_NODES	   0x0000000000001000	/* Use partition nodes
							 * only */
#define RESERVE_FLAG_NO_PART_NODES 0x0000000000002000	/* Clear partition
							 * nodes only flag */
#define RESERVE_FLAG_OVERLAP	   0x0000000000004000	/* Permit to overlap
							 * others */
#define RESERVE_FLAG_SPEC_NODES	   0x0000000000008000	/* Contains specific
							 * nodes */
#define RESERVE_FLAG_FIRST_CORES   0x0000000000010000	/* Use only first cores
							 * on each node */
#define RESERVE_FLAG_TIME_FLOAT	   0x0000000000020000	/* Time offset is
							 * relative */
#define RESERVE_FLAG_REPLACE	   0x0000000000040000	/* Replace resources
							 * as assigned to jobs */
#define RESERVE_FLAG_ALL_NODES	   0x0000000000080000	/* Use all compute
							 * nodes */
#define RESERVE_FLAG_PURGE_COMP	   0x0000000000100000	/* Purge reservation
							 * after last job done */
#define RESERVE_FLAG_WEEKDAY	   0x0000000000200000	/* Set WEEKDAY flag */
#define RESERVE_FLAG_NO_WEEKDAY	   0x0000000000400000	/* Clear WEEKDAY flag */
#define RESERVE_FLAG_WEEKEND	   0x0000000000800000	/* Set WEEKEND flag */
#define RESERVE_FLAG_NO_WEEKEND	   0x0000000001000000	/* Clear WEEKEND flag */
#define RESERVE_FLAG_FLEX	   0x0000000002000000	/* Set FLEX flag */
#define RESERVE_FLAG_NO_FLEX	   0x0000000004000000	/* Clear FLEX flag */
#define RESERVE_FLAG_DUR_PLUS	   0x0000000008000000	/* Add duration time,
							 * only used on
							 * modifying a
							 * reservation */
#define RESERVE_FLAG_DUR_MINUS	   0x0000000010000000	/* Remove duration time,
							 * only used on
							 * modifying a
							 * reservation */
#define RESERVE_FLAG_NO_HOLD_JOBS  0x0000000020000000	/* No hold jobs after
							 * end of reservation */
#define RESERVE_FLAG_REPLACE_DOWN  0x0000000040000000	/* Replace DOWN or
							 * DRAINED nodes */
#define RESERVE_FLAG_NO_PURGE_COMP 0x0000000080000000	/* Clear PURGE flag */

#define RESERVE_FLAG_MAGNETIC	   0x0000000100000000	/* Allow jobs to run
							 * without specifying
							 * the reservation name
							 * if they meet
							 * eligibility status */
#define RESERVE_FLAG_NO_MAGNETIC   0x0000000200000000	/* Clear MAGNETIC flag
							 */
#define RESERVE_FLAG_SKIP	   0x0000000400000000	/* Skip/delete
							 * next/current
							 * reservation without
							 * deleting the
							 * reservation proper */
...
#define DEBUG_FLAG_NET_RAW	0x0000000000000010 /* Raw Network dumps */
...
#define DEBUG_FLAG_WORKQ 	0x0000000000000200 /* Work Queue */
...
#define DEBUG_FLAG_HETJOB	0x0002000000000000 /* Heterogeneous job debug */
#define DEBUG_FLAG_ACCRUE       0x0004000000000000 /* Accrue counters debug */
#define DEBUG_FLAG_POWER_SAVE   0x0008000000000000 /* Power Save debug */
#define DEBUG_FLAG_AGENT	0x0010000000000000 /* RPC Agent debug */
#define DEBUG_FLAG_DEPENDENCY	0x0020000000000000 /* Dependency debug */
...
typedef struct {
	/* Added by Yiqin Dai */
	uint16_t mediate_cnt;
	char** mediate_machine;
	char** mediate_addr;
	uint16_t mediate_width;    /* number of threads per mediate node to span */
	/* Added by Yiqin Dai (end) */
...
#define KILL_JOB_RESV	 0x0100	/* Job is willing to run on nodes in a
				 * magnetic reservation. */
...
/* Added by Yiqin Dai */
extern int slurm_load_mediate_node(mediate_node_info_msg_t* resp);
/* Added by Yiqin Dai (end) */
