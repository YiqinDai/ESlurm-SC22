 ...
 * Functions *
 *************/
static void print_mediate_node_info(mediate_node_info_msg_t* resp); 
...
{
	printf("\n");
	printf("MEDIATE_NODES    STATE      NODELIST\n");
	
	if(resp->normal_mediate_cnt > 0 && resp->normal_mediate_cnt < 10)
		printf("            %d    RUNNING    %s\n",resp->normal_mediate_cnt,resp->normal_mediate_nodelist);
	else if(resp->normal_mediate_cnt >= 10)
		printf("           %d    RUNNING    %s\n",resp->normal_mediate_cnt,resp->normal_mediate_nodelist);
	else;
	
	if(resp->wrong_mediate_cnt > 0 && resp->wrong_mediate_cnt < 10)
		printf("            %d    FAULT      %s\n",resp->wrong_mediate_cnt,resp->wrong_mediate_nodelist);
	else if(resp->wrong_mediate_cnt >= 10)
		printf("           %d    FAULT      %s\n",resp->wrong_mediate_cnt,resp->wrong_mediate_nodelist);
	else;

	if(resp->busy_mediate_cnt > 0 && resp->busy_mediate_cnt < 10)
		printf("            %d    BUSY       %s\n",resp->busy_mediate_cnt,resp->busy_mediate_nodelist);
	else if(resp->busy_mediate_cnt >= 10)
		printf("           %d    BUSY       %s\n",resp->busy_mediate_cnt,resp->busy_mediate_nodelist);
	else;
	printf("\n");
	return;
		     ...
		     char *cluster_name)
{
    ...
    if(params.mediate_info)
    {
    	mediate_node_info_msg_t* resp = xmalloc(sizeof(mediate_node_info_msg_t));
    	if(slurm_load_mediate_node(resp) >= 0)
    		if (resp->normal_mediate_cnt > 0 || resp->wrong_mediate_cnt > 0 || resp->busy_mediate_cnt > 0)
    			print_mediate_node_info(resp);
    	if(resp)
    	{
    		if(resp->normal_mediate_nodelist)
    			xfree(resp->normal_mediate_nodelist);
    		if(resp->wrong_mediate_nodelist)
    			xfree(resp->wrong_mediate_nodelist);
    		if(resp->busy_mediate_nodelist)
    			xfree(resp->busy_mediate_nodelist);
    		xfree(resp);
    	}
    }
