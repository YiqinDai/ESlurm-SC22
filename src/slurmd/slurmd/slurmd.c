...
#define AGENT_THREAD_COUNT	50	
static int ifmediate;
...
typedef struct resp_multiple {
    hostlist_t comm_err_hostlist;
    uint16_t signal_tasks_complete;
    uint16_t ret_cnt;
    uint16_t version;
    List ret_list;
    time_t start_time;
}resp_mtp_t;
typedef struct resp_single {
    uint32_t ret_job_id;
    uint32_t ret_step_id;

    uint16_t no_resp_flag;
    hostlist_t retry_hostlist;

    uint16_t no_resp_cnt;
    uint32_t* no_resp_start_time;
    uint16_t* no_resp_type;
    hostlist_t no_resp_hostlist;

    hostlist_t failed_hostlist;
    hostlist_t dupid_hostlist;
    hostlist_t done_hostlist;
    hostlist_t error_hostlist;
}resp_sg_t;
typedef struct agent_info {
    ...
    resp_mtp_t *response_multiple_struct;
    resp_sg_t response_single_struct;
...
typedef struct task_info {
    ...
    pthread_cond_t *thread_cond_ptr;/* pointer to agent specific
    				 * condition */
    pthread_mutex_t *response_mutex_ptr;
    ...
    uint16_t protocol_version;	/* if set, use this version */
    resp_mtp_t *response_multiple_struct_ptr;
} task_info_t;
...
static void      _handle_connection2(int fd, slurm_addr_t *client);
static void      _msg_engine2(void);
static void     *_service_connection2(void *);
static void      _update_logging2(void);
static void _update_wdog_state(thd_t *thread_ptr, state_t *state, thd_complete_t *thd_comp);
static void _notify_slurmctld_jobs(agent_info_t *agent_ptr);
static int _setup_requeue(agent_info_t * agent_ptr, thd_t *thread_ptr, int *count, int *spot);
static void _queue_agent_retry(agent_info_t * agent_ptr, int count);
static void _notify_slurmctld_nodes(agent_info_t *agent_ptr, int thd_no_resp_cnt, int retry_cnt);
static void *_wdog(void *args);
static void _sig_handler(int dummy);
static void *_thread_per_group_mediate_rpc(void *args);
static agent_info_t *_mediate_make_agent_info(agent_arg_t *agent_arg_ptr);
static task_info_t *_make_task_data(agent_info_t *agent_ptr, int inx);
void gather_response_msg(agent_info_t *agent_ptr,agent_response_t *agent_response);
static int _valid_agent_arg(agent_arg_t *agent_arg_ptr);
static void mediate_req(slurm_msg_t *msg,agent_t *agent_receive);
...
static void _notify_slurmctld_jobs(agent_info_t *agent_ptr)
{
	...
	{
		agent_ptr->response_single_struct.ret_job_id = job_id;
		agent_ptr->response_single_struct.ret_step_id = step_id;
	}*/
	error("%s: invalid msg_type %u", __func__, agent_ptr->msg_type);
}
...
static int _setup_requeue(agent_info_t * agent_ptr,thd_t *thread_ptr, int *count, int *spot)
{
	...
	while ((ret_data_info = list_next(itr)))
	{
		...
		hostlist_push_host(agent_ptr->response_single_struct.retry_hostlist, ret_data_info->node_name);
		++(*spot);
		if (*spot == *count) {
			rc = 1;
			break;
...
static void _queue_agent_retry(agent_info_t * agent_ptr, int count)
{
	...
	agent_arg_ptr->msg_type = agent_ptr->msg_type;
	agent_arg_ptr->msg_args = *(agent_ptr->msg_args_pptr);
	*(agent_ptr->msg_args_pptr) = NULL;*/
	...
	for (i = 0; i < agent_ptr->thread_count; i++)
	{
		if (!thread_ptr[i].ret_list)
		{
			...
			hostlist_push_host(agent_ptr->response_single_struct.retry_hostlist, thread_ptr[i].nodelist);
			j++;
		}
		if (j == count)
			break;
		else {
			if (_setup_requeue(agent_ptr, &thread_ptr[i], &count, &j))
				break;
		}
	}
	/*if (count == 0) {
		...
		*(agent_ptr->msg_args_pptr) = agent_arg_ptr->msg_args;
		xfree(agent_arg_ptr);
		return;
	}*/
	if (count != j) {
...
static void _notify_slurmctld_nodes(agent_info_t *agent_ptr, int thd_no_resp_cnt, int retry_cnt)
{
	...
	char *node_names = xcalloc(100,sizeof(char));
	if (thd_no_resp_cnt) {
		if (agent_ptr->msg_type == REQUEST_BATCH_JOB_LAUNCH) {
			agent_ptr->response_single_struct.no_resp_flag = 1;
		}
		agent_ptr->response_single_struct.no_resp_start_time = xcalloc(thd_no_resp_cnt+2,sizeof(time_t));
		agent_ptr->response_single_struct.no_resp_type = xcalloc(thd_no_resp_cnt+2,sizeof(int));
	}
	...
	for (i = 0; i < agent_ptr->thread_count; i++) {
		...
		is_ret_list = 1;
		agent_ptr->response_multiple_struct[i].ret_list = thread_ptr[i].ret_list; 
		agent_ptr->response_multiple_struct[i].ret_cnt = list_count(thread_ptr[i].ret_list); 
		agent_ptr->response_multiple_struct[i].version = agent_ptr->protocol_version;
		...
		switch_on_state:
			if (is_ret_list) {
				strcpy(node_names,ret_data_info->node_name);
			...
			{
			case DSH_NO_RESP:
				hostlist_push_host(agent_ptr->response_single_struct.no_resp_hostlist, node_names);
				agent_ptr->response_single_struct.no_resp_start_time[agent_ptr->response_single_struct.no_resp_cnt] = thread_ptr[i].start_time;
				agent_ptr->response_single_struct.no_resp_type[agent_ptr->response_single_struct.no_resp_cnt] = resp_type;
				agent_ptr->response_single_struct.no_resp_cnt++;
				debug3("_notify_slurmctld_nodes: the node:%s no resp",node_names);
				break;
			case DSH_FAILED:
				hostlist_push_host(agent_ptr->response_single_struct.failed_hostlist, node_names);
				debug3("_notify_slurmctld_nodes: the node:%s failed",node_names);
				break;
			case DSH_DUP_JOBID:
				hostlist_push_host(agent_ptr->response_single_struct.dupid_hostlist, node_names);
				debug3("_notify_slurmctld_nodes: the node:%s with dupid jobid",node_names);
				break;
			case DSH_DONE:
				hostlist_push_host(agent_ptr->response_single_struct.done_hostlist, node_names);
				debug4("_notify_slurmctld_nodes: the node:%s done",node_names);
				break;
			default:
				hostlist_push_host(agent_ptr->response_single_struct.error_hostlist, node_names);
				debug3("_notify_slurmctld_nodes: the node:%s error",node_names);
				break;
			}
			if (!is_ret_list)
				goto finished;
...
static void *_wdog(void *args)
{
	...
	while (1)
	{
		...
		for (i = 0; i < agent_ptr->thread_count; i++) {
			if (!thread_ptr[i].ret_list) {
				_update_wdog_state(&thread_ptr[i], &thread_ptr[i].state, &thd_comp);
			} else {
				itr = list_iterator_create(thread_ptr[i].ret_list);
				while ((ret_data_info = list_next(itr))) {
					_update_wdog_state(&thread_ptr[i], &ret_data_info->err, &thd_comp); }
				list_iterator_destroy(itr);
			}
		}
	...
	if(thd_comp.max_delay )
		info("%s: agent maximum delay %d seconds", __func__, thd_comp.max_delay);
...
static void *_thread_per_group_mediate_rpc(void *args)
{
	...
	pthread_mutex_t *response_mutex_ptr = task_ptr->response_mutex_ptr;
	uint32_t        *threads_active_ptr = task_ptr->threads_active_ptr;
	thd_t           *thread_ptr = task_ptr->thread_struct_ptr;
	resp_mtp_t *response_multiple_ptr = task_ptr->response_multiple_struct_ptr;
	...
	slurm_mutex_lock(response_mutex_ptr);
	response_multiple_ptr->start_time = time(NULL);
	slurm_mutex_unlock(response_mutex_ptr);
	...
	debug3("%s: sending %s to %s", __func__, rpc_num2string(msg_type),thread_ptr->nodelist);
	if (task_ptr->get_reply) 
	{
		if (thread_ptr->addr)
		{
			...
			debug("important case: get_reply but have addr");
			debug3("successfully send msg to the compute nodes by slurm_send_addr_recv_msgs");
		}
		else
		{
			if(msg_type == REQUEST_PING || msg_type == REQUEST_NODE_REGISTRATION_STATUS)
			{	
				if (!(ret_list = mediate_slurm_send_recv_msgs(thread_ptr->nodelist, &msg, 30000)))
				{
					error("%s: no ret_list given", __func__);
					goto cleanup;
				}
				debug3("successfully send msg to the compute nodes by mediate_slurm_sned_recv_msgs");
			}
			else
			{
				if (!(ret_list = mediate_slurm_send_recv_msgs(thread_ptr->nodelist, &msg, 0)))
				{
					error("%s: no ret_list given", __func__);
					goto cleanup;
				}
	...
	else 
	{
		if (thread_ptr->addr)
			...
			if (slurm_conf_get_addr(thread_ptr->nodelist,
				&msg.address, msg.flags) == SLURM_ERROR) {
		...
		if (msg_type == SRUN_JOB_COMPLETE) {
			...
			debug3("successfully send msg to the compute nodes by slurm_send_msg_maybe");
			thread_state = DSH_DONE;
		} else if (slurm_send_only_node_msg(&msg) == SLURM_SUCCESS) {
			debug3("successfully send msg to the compute nodes by slurm_send_only_node_msg");
			thread_state = DSH_DONE;
		}
		else {
			if (!srun_agent) {
				slurm_mutex_lock(response_mutex_ptr);
				hostlist_t comm_err_hostlist_tmp = hostlist_create(thread_ptr->nodelist);
				char *node_name = NULL;
				while ((node_name = hostlist_shift(comm_err_hostlist_tmp))) {
					debug("important case: we have comm_err_hostlist");
					hostlist_push_host(response_multiple_ptr->comm_err_hostlist, node_name);
					free(node_name);
				}
				hostlist_destroy(comm_err_hostlist_tmp);
				slurm_mutex_unlock(response_mutex_ptr);
			}
		}
		goto cleanup;
		...
		while ((ret_data_info = list_next(itr))) {
			...
			default:
				if (srun_agent)
					...
					info("important case: need to process");
					thread_state = DSH_DONE;
				}
			}
...
cleanup:
	...
	if (!ret_list && (msg_type == REQUEST_SIGNAL_TASKS)) {
		...
		debug("important  case: signal tasks no return list");
		if ((msg_ptr->signal == SIGCONT) ||
			(msg_ptr->signal == SIGSTOP)) {
			slurm_mutex_lock(response_mutex_ptr);
			response_multiple_ptr->signal_tasks_complete = 1;
			slurm_mutex_unlock(response_mutex_ptr);
		}
	}
	...
	thread_ptr->end_time = (time_t)difftime(time(NULL), thread_ptr->start_time);
...
{
	int i = 0, j = 0;
	agent_info_t *agent_ptr = NULL;
	thd_t *thread_ptr = NULL;
	resp_mtp_t *resp_multiple_ptr = NULL;
	int *span = NULL;
	int thr_count = 0;
	hostlist_t hl = NULL;
	char *name = NULL;

	agent_ptr = xmalloc(sizeof(agent_info_t));
	slurm_mutex_init(&agent_ptr->thread_mutex);
	slurm_mutex_init(&agent_ptr->response_mutex);
	slurm_cond_init(&agent_ptr->thread_cond, NULL);
	agent_ptr->thread_count = agent_arg_ptr->node_count;
	agent_ptr->retry = agent_arg_ptr->retry;
	agent_ptr->threads_active = 0;

	thread_ptr = xcalloc(agent_ptr->thread_count, sizeof(thd_t));
	agent_ptr->thread_struct = thread_ptr;
	

	agent_ptr->msg_type = agent_arg_ptr->msg_type;
	agent_ptr->msg_args_pptr = &agent_arg_ptr->msg_args;
	agent_ptr->protocol_version = agent_arg_ptr->protocol_version;
	...
	i = 0;
	while (i < agent_ptr->thread_count) {
		...
		//hostlist_uniq_without_qsort(hl);
		thread_ptr[thr_count].nodelist =
			hostlist_ranged_string_xmalloc(hl);
		//debug3("==================%d thread's nodelist = %s",thr_count,thread_ptr[thr_count].nodelist);
		hostlist_destroy(hl);
	...
	xfree(span);
	agent_ptr->thread_count = thr_count;
	/*resp_multiple_ptr*/
	resp_multiple_ptr = xcalloc(agent_ptr->thread_count, sizeof(resp_mtp_t));
	for(int ll=0;ll<agent_ptr->thread_count;ll++)
	{
		resp_multiple_ptr[ll].comm_err_hostlist = hostlist_create(NULL);
		resp_multiple_ptr[ll].ret_list = list_create(NULL);
		resp_multiple_ptr[ll].ret_cnt = 0;
		
		resp_multiple_ptr[ll].signal_tasks_complete = 0;
		resp_multiple_ptr[ll].version = 0;
		resp_multiple_ptr[ll].start_time = 0;
	}
	agent_ptr->response_multiple_struct = resp_multiple_ptr;
	/*resp_single_ptr*/
	agent_ptr->response_single_struct.ret_job_id = NO_VAL;
	agent_ptr->response_single_struct.ret_step_id = NO_VAL;
	agent_ptr->response_single_struct.no_resp_flag = 0;
	agent_ptr->response_single_struct.retry_hostlist = hostlist_create(NULL);
	agent_ptr->response_single_struct.no_resp_cnt = 0;
	agent_ptr->response_single_struct.no_resp_start_time = NULL;
	agent_ptr->response_single_struct.no_resp_type = NULL;
	agent_ptr->response_single_struct.no_resp_hostlist = hostlist_create(NULL);
	agent_ptr->response_single_struct.failed_hostlist = hostlist_create(NULL);
	agent_ptr->response_single_struct.dupid_hostlist = hostlist_create(NULL);
	agent_ptr->response_single_struct.done_hostlist = hostlist_create(NULL);
	agent_ptr->response_single_struct.error_hostlist = hostlist_create(NULL);
...
static task_info_t *_make_task_data(agent_info_t *agent_ptr, int inx)
{
	...
	task_info_ptr->thread_mutex_ptr = &agent_ptr->thread_mutex;
	task_info_ptr->response_mutex_ptr = &agent_ptr->response_mutex;
	task_info_ptr->thread_cond_ptr = &agent_ptr->thread_cond;
	task_info_ptr->threads_active_ptr = &agent_ptr->threads_active;
	task_info_ptr->thread_struct_ptr = &agent_ptr->thread_struct[inx];
	task_info_ptr->get_reply = agent_ptr->get_reply;
	task_info_ptr->msg_type = agent_ptr->msg_type;
	task_info_ptr->msg_args_ptr = *agent_ptr->msg_args_pptr;
	task_info_ptr->protocol_version = agent_ptr->protocol_version;
	task_info_ptr->response_multiple_struct_ptr = &agent_ptr->response_multiple_struct[inx];
	return task_info_ptr;
}
void gather_response_msg(agent_info_t *agent_ptr, agent_response_t *agent_response)
{
	debug3("start to gather response msg:");
	hostlist_t total_comm_err_hostlist = hostlist_create(NULL);
	for (int i = 0; i < agent_ptr->thread_count; i++)
	{
		if (hostlist_count(agent_ptr->response_multiple_struct[i].comm_err_hostlist)>0)
		{
			char* name = NULL;
			agent_response->comm_err_flag = 1;
			while ((name = hostlist_shift(agent_ptr->response_multiple_struct[i].comm_err_hostlist)))
			{
				hostlist_push_host(total_comm_err_hostlist, name);
				debug("important case: the node:%s is added to the comm_err_hostlist",name);
				free(name);
			}
		}
	}
	if (hostlist_count(total_comm_err_hostlist)!= 0)
	{
		agent_response->comm_err_nodelist = hostlist_ranged_string_xmalloc(total_comm_err_hostlist);
	}
	hostlist_destroy(total_comm_err_hostlist);
	for (int i = 0; i < agent_ptr->thread_count; i++)
	{
		if (agent_ptr->response_multiple_struct[i].signal_tasks_complete > 0)
		{
			agent_response->signal_tasks_complete = agent_ptr->response_multiple_struct[i].signal_tasks_complete;
			debug("important case: gather signal_tasks_complete");
		}
	}
	if (agent_ptr->response_single_struct.ret_job_id != NO_VAL)
	{
		agent_response->job_id = agent_ptr->response_single_struct.ret_job_id;
		agent_response->step_id = agent_ptr->response_single_struct.ret_step_id;
		debug("important case: gather job_id = %d, step_id= %d", agent_response->job_id, agent_response->step_id);
	}
	if (agent_ptr->response_single_struct.no_resp_flag > 0)
	{
		agent_response->no_resp_flag = agent_ptr->response_single_struct.no_resp_flag;
		debug("important case: gather no_resp_flag");
	}
	if (hostlist_count(agent_ptr->response_single_struct.retry_hostlist)!=0)
	{
		agent_response->retry_flag = 1;
		agent_response->retry_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.retry_hostlist);
		debug("important case: gather retry nodelist");
	}
	if (agent_ptr->response_single_struct.no_resp_cnt > 0)
	{
		agent_response->no_resp_cnt = agent_ptr->response_single_struct.no_resp_cnt;
		agent_response->no_resp_start_time = agent_ptr->response_single_struct.no_resp_start_time;
		agent_response->type = agent_ptr->response_single_struct.no_resp_type;
		agent_response->no_resp_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.no_resp_hostlist);
		
		debug3("gather no_resp_nodelist");
	}
	int error_flag = 0;
	if (hostlist_count(agent_ptr->response_single_struct.failed_hostlist)!=0)
	{
		error_flag += 8;
		agent_response->failed_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.failed_hostlist);
		debug3("gather failed nodelist");
	}
	if (hostlist_count(agent_ptr->response_single_struct.dupid_hostlist)!=0)
	{
		error_flag += 4;
		agent_response->dupid_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.dupid_hostlist);
		debug3("gather dupid nodelist*");
	}
	if (hostlist_count(agent_ptr->response_single_struct.done_hostlist)!=0)
	{
		error_flag += 2;
		agent_response->done_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.done_hostlist);
		debug3("gather done nodelist");
	}
	if (hostlist_count(agent_ptr->response_single_struct.error_hostlist)!=0)
	{
		error_flag += 1;
		agent_response->error_nodelist = hostlist_ranged_string_xmalloc(agent_ptr->response_single_struct.error_hostlist);
		
		debug3("gather error nodelist");
	}
	agent_response->error_flag = error_flag;
	debug3("gather error_flag = %d",error_flag);
	agent_response->start_time = time(NULL);
	
	for (int i = 0; i < agent_ptr->thread_count; i++)
	{
		if (agent_ptr->response_multiple_struct[i].ret_cnt != 0)
		{
			agent_response->ret_cnt += agent_ptr->response_multiple_struct[i].ret_cnt;
			list_append_list(agent_response->ret_list, agent_ptr->response_multiple_struct[i].ret_list);
			agent_response->version = agent_ptr->response_multiple_struct[i].version;
		}
		agent_response->start_time = agent_response->start_time < agent_ptr->response_multiple_struct[i].start_time ? agent_response->start_time : agent_ptr->response_multiple_struct[i].start_time;
	}
	debug3("the count of whole agent_response->ret_list  = %d",list_count(agent_response->ret_list));
	return;
}
...
void mediate_req(slurm_msg_t *msg,agent_t *agent_receive)
{
	if(msg->msg_type == REQUEST_PING_MEDIATE)
	{
		slurm_msg_t *resp_msg =  xmalloc(sizeof(slurm_msg_t));
		slurm_msg_t_init(resp_msg);
		resp_msg->msg_type = RESPONSE_PING_MEDIATE;
	...
	debug3("agent_receive's nodelist  = %s ",agent_receive->nodelist);
	agent_arg_t* agent_arg_ptr = xmalloc(sizeof(agent_arg_t));
	agent_arg_ptr->node_count = agent_receive->node_count;
	agent_arg_ptr->retry = agent_receive->retry;
	agent_arg_ptr->addr = agent_receive->addr;
	agent_arg_ptr->hostlist = hostlist_create(agent_receive->nodelist);
	agent_arg_ptr->protocol_version = agent_receive->protocol_version;
	agent_arg_ptr->msg_type = msg->msg_type;
	agent_arg_ptr->msg_args = msg->data;
	msg->data = NULL;
	...
	agent_ptr = _mediate_make_agent_info(agent_arg_ptr);
	thread_ptr = agent_ptr->thread_struct;

	slurm_thread_create(&thread_wdog, _wdog, agent_ptr);

	debug("New request: msg_type = %s, node_count = %d",rpc_num2string(agent_arg_ptr->msg_type), agent_arg_ptr->node_count);
	
	debug3("%s: New agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
		__func__, agent_ptr->thread_count,
		agent_ptr->threads_active,
		agent_ptr->retry ? 'T' : 'F',
		agent_ptr->get_reply ? 'T' : 'F',
		rpc_num2string(agent_arg_ptr->msg_type),
		agent_ptr->protocol_version);
	...
	for (i = 0; i < agent_ptr->thread_count; i++)
	{
		...
		slurm_mutex_lock(&agent_ptr->thread_mutex);
		while (agent_ptr->threads_active >=
			AGENT_THREAD_COUNT)
		{
			slurm_cond_wait(&agent_ptr->thread_cond,
				&agent_ptr->thread_mutex);
		}
		 ...
		 */
		task_specific_ptr = _make_task_data(agent_ptr, i);

		slurm_thread_create_detached(&thread_ptr[i].thread,
			_thread_per_group_mediate_rpc,
			task_specific_ptr);
		agent_ptr->threads_active++;
		slurm_mutex_unlock(&agent_ptr->thread_mutex);
	}
	...
	info("agent msg_type=%u ran for %d seconds", agent_arg_ptr->msg_type, delay);
	...
	slurm_mutex_unlock(&agent_ptr->thread_mutex);
	debug3("%s: end agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
		__func__, agent_ptr->thread_count,
		agent_ptr->threads_active,
		agent_ptr->retry ? 'T' : 'F',
		agent_ptr->get_reply ? 'T' : 'F',
	...
	gather_response_msg(agent_ptr, agent_response);
	
	slurm_msg_t_init(&resp_msg);
	resp_msg.msg_type = MEDIATE_RESPONSE_CTLD;
	resp_msg.data = (agent_response_t*)agent_response;
	slurm_send_node_msg(msg->conn_fd, &resp_msg);
	debug3("successfully send back to the slurmctld");
	resp_msg.data = NULL;
...
cleanup:
	...
	for (int i = 0; i < agent_ptr->thread_count; i++)
	{
		if(agent_ptr->thread_struct[i].ret_list)
			FREE_NULL_LIST(agent_ptr->thread_struct[i].ret_list);
		if(agent_ptr->thread_struct[i].nodelist)
			xfree(agent_ptr->thread_struct[i].nodelist);
		hostlist_destroy(agent_ptr->response_multiple_struct[i].comm_err_hostlist);
	...
	if (agent_ptr)
	{
		if(agent_ptr->thread_struct)
			xfree(agent_ptr->thread_struct);
		hostlist_destroy(agent_ptr->response_single_struct.retry_hostlist);
		hostlist_destroy(agent_ptr->response_single_struct.no_resp_hostlist);
		hostlist_destroy(agent_ptr->response_single_struct.failed_hostlist);
		hostlist_destroy(agent_ptr->response_single_struct.dupid_hostlist);
		hostlist_destroy(agent_ptr->response_single_struct.done_hostlist);
		hostlist_destroy(agent_ptr->response_single_struct.error_hostlist);
		if(agent_ptr->response_multiple_struct)
			xfree(agent_ptr->response_multiple_struct);
		if(agent_ptr)
			xfree(agent_ptr);
	}
	return;
	...
	if (!ifmediate)
	{
		...
		*/
		...
		if (!gethostname(host, MAXHOSTNAMELEN)) 
			...
			fatal("cannot find the mediate hostname");
		...
		if (pidfd >= 0)			/* valid pidfd, non-error */
			(void)close(pidfd);	/* Ignore errors */
		if (unlink(slurm_conf.slurmd_pidfile) < 0)
			error("Unable to remove pidfile `%s': %m", slurm_conf.slurmd_pidfile);
...
_msg_engine2(void)
{
	...
	while (!_shutdown)
	{
		...
		{
			debug3("the mediate node receives a request from slurmctld");
			_handle_connection2(sock, cli);
			continue;
		}
		/*
...
static void _handle_connection2(int fd, slurm_addr_t *cli)
{
	...
	_increment_thd_count();
	slurm_thread_create_detached(NULL, _service_connection2, arg);
}
...
static void *
_service_connection2(void *arg)
{
	conn_t *con = (conn_t *)arg;
	slurm_msg_t *msg = xmalloc(sizeof(slurm_msg_t));
	int rc = SLURM_SUCCESS;

	debug("in the service_connection2");
	slurm_msg_t_init(msg);

	agent_t* agent_receive = xmalloc(sizeof(agent_t));
	agent_t_init(agent_receive);

	if ((rc = slurm_mediate_receive_msg(con->fd, con->cli_addr, msg, agent_receive, 0)) != SLURM_SUCCESS)
	{
		error("service_connection2: slurm_mediate_receive_msg: %m");
		goto cleanup;
	}
	debug("Start processing RPC: %s", rpc_num2string(msg->msg_type));
	mediate_req(msg, agent_receive);
cleanup:
	if ((msg->conn_fd >= 0) && close(msg->conn_fd) < 0)
		error("close(%d): %m", con->fd);
	xfree(con->cli_addr);
	xfree(con);
	debug("Finish processing RPC: %s", rpc_num2string(msg->msg_type));
	slurm_free_msg(msg);
	_decrement_thd_count();
	xfree(agent_receive->nodelist);
	xfree(agent_receive);
	return NULL;
}
...
_service_connection(void *arg)
{
	...
	if(ifmediate)
		fatal("The mediate node start the normal process!"); 
...
_read_config(void)
{
	...
	int error_code;
	if ((error_code = gethostname_short(node_name_short,
		MAX_SLURM_NAME)))
		fatal("getnodename_short error %s", slurm_strerror(error_code));

	if ((error_code = gethostname(node_name_long,
		MAX_SLURM_NAME)))
		fatal("getnodename error %s", slurm_strerror(error_code));
	int i = 0;
	for (i = 0; i < cf->mediate_cnt; i++){
		if (cf->mediate_machine[i] && cf->mediate_addr[i] &&
			(!xstrcmp(node_name_short, cf->mediate_machine[i]) ||
				!xstrcmp(node_name_long, cf->mediate_machine[i])))
			break;
	}
	if (i < cf->mediate_cnt){
		conf->port = cf->slurmd_port;
		conf->node_name = conf->hostname = cf->mediate_machine[i];
		ifmediate = 1;
	}else{
		ifmediate = 0;
	}
	if (!ifmediate){
		...
		*/
		...
		info("Caution! This is a Mediate Node");
		_massage_pathname(&conf->logfile);
		/* store hardware properties in slurmd_config */
		xfree(conf->block_map);
		xfree(conf->block_map_inv);
