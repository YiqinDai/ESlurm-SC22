...
pthread_mutex_t mediate_pthread_cnt_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t mediate_pthread_cnt_cond = PTHREAD_COND_INITIALIZER;
int mediate_pthread_count = 0;
static void _mediate_start_msg_tree_internal(hostlist_t hl, hostlist_t* sp_hl,
...
void *_mediate_fwd_tree_thread(void *arg)
{
	...
	while ((name = hostlist_shift(fwd_tree->tree_hl))) {
		...
		ret_list = mediate_slurm_send_addr_recv_msgs(&send_msg, name,
						     fwd_tree->timeout);
		...
		if (ret_list) {
			...
			*/
			...
			if (ret_cnt <= send_msg.forward.cnt) {
				 ...
				 * node serially.
				 */
				debug("Warning, the tree is abandoned! %d msg will be sent from this mediate node!", hostlist_count(fwd_tree->tree_hl));
				_mediate_start_msg_tree_internal(
	...
	slurm_mutex_lock(&mediate_pthread_cnt_mutex);
	mediate_pthread_count -= 1;
	debug("mediate_pthread_count = %d", mediate_pthread_count);
	slurm_cond_broadcast(&mediate_pthread_cnt_cond);
	slurm_mutex_unlock(&mediate_pthread_cnt_mutex);	
	return NULL;
}
				     ...
				     int hl_count)
{
	...
	debug("msg_type=%d, hl_count=%d",fwd_tree_in->orig_msg->msg_type, hl_count);
	
	slurm_mutex_lock(&mediate_pthread_cnt_mutex);
	while(1)
	{
		if(mediate_pthread_count + hl_count <= 8000)
		{
			mediate_pthread_count += hl_count;
			debug("The pthread_count is added by %d to %d", hl_count, mediate_pthread_count); 
			break;
		}
		else
			slurm_cond_wait(&mediate_pthread_cnt_cond, &mediate_pthread_cnt_mutex);
	}
	slurm_mutex_unlock(&mediate_pthread_cnt_mutex);
	...
	for (j = 0; j < hl_count; j++) {
		...
		debug3("%d nodelist = %s",j,hostlist_ranged_string_xmalloc(fwd_tree->tree_hl));
		/*
		...
		slurm_thread_create_detached(NULL, _mediate_fwd_tree_thread, fwd_tree);
		usleep(100);
	}
}
...
extern List mediate_start_msg_tree(hostlist_t hl, slurm_msg_t *msg, int timeout, uint16_t tree_width)
{
	fwd_tree_t fwd_tree;
	pthread_mutex_t tree_mutex;
	pthread_cond_t notify;
	...
	if (route_g_split_hostlist(hl, &sp_hl, &hl_count, tree_width))
	{
		error("unable to split forward hostlist");
		return NULL;
	}
	...
	fwd_tree.p_thr_count = &thr_count;//0
	fwd_tree.tree_mutex = &tree_mutex;

	_mediate_start_msg_tree_internal(NULL, sp_hl, &fwd_tree, hl_count);
