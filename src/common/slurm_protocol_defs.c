...
extern void agent_t_init(agent_t *agent_send)
{
	memset(agent_send, 0, sizeof(agent_t));
	agent_send->node_count = 0;
	agent_send->retry = 0;
	agent_send->protocol_version = NO_VAL16;
	agent_send->nodelist = NULL;
	agent_send->addr_flag = 0;
	agent_send->addr = NULL;
}

extern void agent_response_t_init(agent_response_t *agent_response)
{
	memset(agent_response, 0, sizeof(agent_response_t));
	agent_response->comm_err_flag = 0;
	agent_response->comm_err_nodelist = NULL;

	agent_response->signal_tasks_complete = 0;

	agent_response->job_id = agent_response->step_id = NO_VAL;

	agent_response->no_resp_flag = 0;

	agent_response->retry_flag = 0;
	agent_response->retry_nodelist = NULL;

	agent_response->no_resp_cnt = 0;
	agent_response->no_resp_start_time = NULL;
	agent_response->type = NULL;
	agent_response->no_resp_nodelist = NULL;

	agent_response->error_flag = 0;
	agent_response->failed_nodelist = NULL;
	agent_response->dupid_nodelist = NULL;
	agent_response->done_nodelist = NULL;
	agent_response->error_nodelist = NULL;

	agent_response->ret_cnt = 0;
	agent_response->version = NO_VAL16;
	agent_response->ret_list = list_create(NULL);

	agent_response->start_time = 0;
	return;
}


extern void slurm_free_response_ctld(agent_response_t *msg)
{
	if(msg)
	{
		if(msg->comm_err_nodelist)
			xfree(msg->comm_err_nodelist);
		if(msg->retry_nodelist)
			xfree(msg->retry_nodelist);
		if(msg->no_resp_start_time)
			xfree(msg->no_resp_start_time);
		if(msg->type)	
			xfree(msg->type);
		if(msg->no_resp_nodelist)
			xfree(msg->no_resp_nodelist);
		if(msg->failed_nodelist)
			xfree(msg->failed_nodelist);
		if(msg->dupid_nodelist)
			xfree(msg->dupid_nodelist);
		if(msg->done_nodelist)
			xfree(msg->done_nodelist);
		if(msg->error_nodelist)
			xfree(msg->error_nodelist); 
		if(msg->ret_list)
...
extern void slurm_free_mediate_info(mediate_node_info_msg_t* msg)
{
	if(msg)
	{
		if(msg->normal_mediate_nodelist)
			xfree(msg->normal_mediate_nodelist);
		if(msg->wrong_mediate_nodelist)
			xfree(msg->wrong_mediate_nodelist);
		if(msg->busy_mediate_nodelist)
			xfree(msg->wrong_mediate_nodelist);
	}
	xfree(msg);
}
...
extern int slurm_free_msg_data(slurm_msg_type_t type, void *data)
{
	...
	case MEDIATE_RESPONSE_CTLD:
		slurm_free_response_ctld(data);  
		break;
	case RESPONSE_MEDIATE_INFO:
		slurm_free_mediate_info(data);   
		break;	
	default:
...
rpc_num2string(uint16_t opcode)
{
	...
	case PERSIST_RC:
		return "PERSIST_RC";
	case MEDIATE_RESPONSE_CTLD:
		return "MEDIATE_RESPONSE_CTLD";
	case REQUEST_PING_MEDIATE:
		return "REQUEST_PING_MEDIATE";
	case RESPONSE_PING_MEDIATE:
		return "RESPONSE_PING_MEDIATE";
	case REQUEST_MEDIATE_INFO:
		return "REQUEST_MEDIATE_INFO";
	case RESPONSE_MEDIATE_INFO:
		return "RESPONSE_MEDIATE_INFO";	
	default:
