...
int slurm_init_mediate_msg_engine_port(uint16_t port, char* host)
{
	int cc;
	slurm_addr_t addr;
	int i;

	slurm_setup_mediate_sockaddr(&addr, port, host);
	cc = slurm_init_msg_engine(&addr);
	if ((cc < 0) && (port == 0) && (errno == EADDRINUSE)) {
		/* All ephemeral ports are in use, test other ports */
		for (i = 10001; i < 65536; i++) {
...
int slurm_mediate_receive_msg(int fd, slurm_addr_t *orig_addr, slurm_msg_t *msg, agent_t* agent_receive, int timeout)
{
	char *buf = NULL;
	size_t buflen = 0;
	header_t header;
	...
	debug3("msg_type = %s",rpc_num2string(header.msg_type));
	if (check_header_version(&header) < 0) {
		slurm_addr_t resp_addr;
		char addr_str[32];
		int uid = _unpack_msg_uid(buffer, header.version);
	...
	if (header.forward.cnt > 0) {
		error("Mediate node should not receive the msg need to forward");
	}
	...
	if(msg->msg_type == REQUEST_PING_MEDIATE)
		goto total_return;
	if (unpack_agent(agent_receive, buffer) == SLURM_ERROR)
	{
		error("failed unpack the agent");
		rc = SLURM_ERROR;
		goto total_return;
	}
	free_buf(buffer);
...
static void _pack_agent(agent_t* agent_send, header_t *hdr, Buf buffer)
{
	unsigned int tmplen, msglen;

	tmplen = get_buf_offset(buffer);
	pack_agent(agent_send, buffer);
	msglen = get_buf_offset(buffer) - tmplen;
...
int slurm_send_mediate_node_msg(int fd, slurm_msg_t * msg, agent_t* agent_send)
{
	header_t header;
	Buf      buffer;
	int      rc;
	...
	_pack_agent(agent_send, &header, buffer);
...
extern int slurm_mediate_send_recv_msg(int fd, slurm_msg_t *req,
	slurm_msg_t *resp, agent_t* agent_send, int timeout)
{
	int rc = -2;
	...
	if (slurm_send_mediate_node_msg(fd, req, agent_send) >= 0) {
...
static int
_mediate_send_and_recv_msg(int fd, slurm_msg_t *req,
	slurm_msg_t *resp, agent_t* agent_send, int timeout)
{
	int rc = slurm_mediate_send_recv_msg(fd, req, resp, agent_send, timeout);
	(void)close(fd);
	return rc;
}

static List
_mediate_send_and_recv_msgs(int fd, slurm_msg_t *req, int timeout)
{
	List ret_list = NULL;
	int steps = 0;
	int t = 0;
	...
	if (slurm_send_node_msg(fd, req) >= 0) {
		if (req->forward.cnt > 0) {
			...
			timeout = (t * steps);
			if(steps == 0 && req->forward.cnt > 0)
				timeout += t;
			steps++;

			timeout += (t * steps);
		}
		//info("node_cnt = %d, treewidth=%d, timeout=%d, msg_type=%d", req->forward.cnt, req->forward.tree_width, timeout, req->msg_type);
...
int slurm_mediate_send_recv_node_msg(slurm_msg_t *req, slurm_msg_t *resp, agent_t* agent_send, int timeout)
{
	int fd = -1;
	...
	if ((fd = slurm_open_msg_conn(&req->address)) < 0)
		return -3;
	return _mediate_send_and_recv_msg(fd, req, resp, agent_send, timeout);

}

List mediate_slurm_send_recv_msgs(const char *nodelist, slurm_msg_t *msg,
	int timeout)
{
	List ret_list = NULL;
	hostlist_t hl = NULL;
	...
	uint16_t mediate_width = slurm_conf.mediate_width;
	ret_list = mediate_start_msg_tree(hl, msg, timeout, mediate_width);
	hostlist_destroy(hl);

	return ret_list;
}

List mediate_slurm_send_addr_recv_msgs(slurm_msg_t *msg, char *name, int timeout)
{
	...
	if (!(ret_list = _mediate_send_and_recv_msgs(fd, msg, timeout))) {
...
extern int * mediate_set_span(int total, uint16_t tree_width)
{
	int *span = NULL;
	...
	int m = total / tree_width;
	int n = total % tree_width;
...
extern void slurm_setup_mediate_sockaddr(slurm_addr_t *sin, uint16_t port, char* host)
{
...
tryagain:
	...
	while (retry) {
		 ...
		 */
		retry = 0;
		rc = _send_and_recv_msg(fd, request_msg, response_msg, MAX(120 *1000, slurm_conf.msg_timeout * 1000 * 3)); 
