			...
			uint16_t protocol_version)
{
	...
	*msg = tmp_ptr;
	...
	if (protocol_version >= SLURM_20_11_PROTOCOL_VERSION) {
		safe_unpackstr_xmalloc(&tmp_ptr->comment, &uint32_tmp, buffer);
	...
	*build_buffer_ptr = build_ptr;
	...
	if (protocol_version >= SLURM_20_11_PROTOCOL_VERSION) {
		...
		safe_unpackstr_xmalloc(&build_ptr->mpi_params,
				       &uint32_tmp, buffer);
		safe_unpackstr_xmalloc(&temp_str, &uint32_tmp, buffer);
		xfree(temp_str); /* was msg_aggr_params */
		safe_unpack16(&build_ptr->msg_timeout, buffer);
		...
		safe_unpackstr_xmalloc(&build_ptr->route_plugin,
				       &uint32_tmp, buffer);
		safe_unpackstr_xmalloc(&temp_str, &uint32_tmp, buffer);
		xfree(temp_str); /* was salloc_default_command */
		safe_unpackstr_xmalloc(&build_ptr->sbcast_parameters,
				       &uint32_tmp, buffer);
		safe_unpackstr_xmalloc(&build_ptr->sched_params,
				       &uint32_tmp, buffer);
...
void pack_agent(agent_t* agent_send, Buf buffer)
{
	pack32(agent_send->node_count, buffer);
	pack16(agent_send->retry, buffer);
	pack16(agent_send->protocol_version, buffer);
	packstr(agent_send->nodelist, buffer);
	pack16(agent_send->addr_flag, buffer);
	if(agent_send->addr_flag == 1)
		slurm_pack_slurm_addr(agent_send->addr, buffer);
}

int unpack_agent(agent_t* agent_send, Buf buffer)
{
	uint32_t uint32_tmp;	
	safe_unpack32(&agent_send->node_count, buffer);
	safe_unpack16(&agent_send->retry, buffer);
	safe_unpack16(&agent_send->protocol_version, buffer);
	safe_unpackstr_xmalloc(&agent_send->nodelist, &uint32_tmp, buffer);
	safe_unpack16(&agent_send->addr_flag, buffer);
	if(agent_send->addr_flag == 1) {
		agent_send->addr = xmalloc(sizeof(agent_t));\
		slurm_unpack_slurm_addr_no_alloc(agent_send->addr, buffer);
	}

	return SLURM_SUCCESS;
unpack_error:
	error("unpacking agent error!");
	agent_t_init(agent_send) ;
	return SLURM_ERROR;
}
...
static void
_pack_mediate_response_msg(agent_response_t * msg, Buf buffer,
		uint16_t protocol_version)
{
	xassert(msg);
	pack16(msg->comm_err_flag,buffer);
	if(msg->comm_err_flag == 1){
		packstr(msg->comm_err_nodelist, buffer);
	}
		pack16(msg->signal_tasks_complete, buffer);
	
	pack32(msg->job_id, buffer);
	pack32(msg->step_id, buffer);
	
	pack16(msg->no_resp_flag, buffer);

	pack16(msg->retry_flag, buffer);
	if (msg->retry_flag == 1) {
		packstr(msg->retry_nodelist, buffer);
	}
	
	pack16(msg->no_resp_cnt, buffer);
	if (msg->no_resp_cnt > 0) {	
		pack32_array(msg->no_resp_start_time, msg->no_resp_cnt, buffer);
		pack16_array(msg->type, msg->no_resp_cnt, buffer);
		packstr(msg->no_resp_nodelist, buffer);
	}
	pack16(msg->error_flag, buffer);
	if(msg->error_flag >= 0 && msg->error_flag <= 15) {
		char stringx[4];
		itoa(msg->error_flag, stringx, 2);
		if (stringx[0] == '1')
			packstr(msg->failed_nodelist, buffer);
		if (stringx[1] == '1')
			packstr(msg->dupid_nodelist, buffer);
		if (stringx[2] == '1')
			packstr(msg->done_nodelist, buffer);
		if (stringx[3] == '1')
			packstr(msg->error_nodelist, buffer);
	} else {
		error("pack_agent_response: wrong error flag while packing agent_response msg!");
	}
	pack16(msg->ret_cnt, buffer);
	if (msg->ret_cnt > 0) {
		pack16(msg->version, buffer);
		_pack_ret_list(msg->ret_list, msg->ret_cnt, buffer, protocol_version);
	}
	pack_time(msg->start_time, buffer);
}

static int _unpack_mediate_response_msg(agent_response_t ** msg, Buf buffer, uint16_t protocol_version)
{
	agent_response_t *tmp_ptr;
	uint32_t uint32_tmp = 0;
	xassert(msg);
	tmp_ptr = xmalloc(sizeof(agent_response_t));
	agent_response_t_init(tmp_ptr);
	*msg = tmp_ptr;
	safe_unpack16(&tmp_ptr->comm_err_flag, buffer);
	if(tmp_ptr->comm_err_flag == 1) {
		safe_unpackstr_xmalloc(&tmp_ptr->comm_err_nodelist, &uint32_tmp, buffer);
	}
	safe_unpack16(&tmp_ptr->signal_tasks_complete, buffer);
	safe_unpack32(&tmp_ptr->job_id, buffer);
	safe_unpack32(&tmp_ptr->step_id, buffer);
	safe_unpack16(&tmp_ptr->no_resp_flag, buffer);
	safe_unpack16(&tmp_ptr->retry_flag, buffer);
	if (tmp_ptr->retry_flag == 1) {
		safe_unpackstr_xmalloc(&tmp_ptr->retry_nodelist, &uint32_tmp, buffer);
	}

	safe_unpack16(&tmp_ptr->no_resp_cnt, buffer);
	if (tmp_ptr->no_resp_cnt > 0) {
		safe_unpack32_array(&tmp_ptr->no_resp_start_time, &uint32_tmp, buffer);
		safe_unpack16_array(&tmp_ptr->type, &uint32_tmp, buffer);
		if(uint32_tmp != tmp_ptr->no_resp_cnt)
			error("unpack_agent_response : no_resp_nodelist related error");
		safe_unpackstr_xmalloc(&tmp_ptr->no_resp_nodelist, &uint32_tmp, buffer);
	}

	safe_unpack16(&tmp_ptr->error_flag, buffer);
	if (tmp_ptr->error_flag >= 0 && tmp_ptr->error_flag <= 15) {
		char stringx[4];
		itoa(tmp_ptr->error_flag, stringx, 2);
		if (stringx[0] == '1')
			safe_unpackstr_xmalloc(&tmp_ptr->failed_nodelist, &uint32_tmp, buffer);
		if (stringx[1] == '1')
			safe_unpackstr_xmalloc(&tmp_ptr->dupid_nodelist, &uint32_tmp, buffer);
		if (stringx[2] == '1')
		{
			safe_unpackstr_xmalloc(&tmp_ptr->done_nodelist, &uint32_tmp, buffer);
		}
		if (stringx[3] == '1')
			safe_unpackstr_xmalloc(&tmp_ptr->error_nodelist, &uint32_tmp, buffer);
	} else {
		error("unpack_agent_response: wrong error flag while unpacking agent_response msg!");
	}

	safe_unpack16(&tmp_ptr->ret_cnt, buffer);
	if (tmp_ptr->ret_cnt > 0) {
		
		safe_unpack16(&tmp_ptr->version, buffer);
		if (_unpack_ret_list(&(tmp_ptr->ret_list), tmp_ptr->ret_cnt, buffer, protocol_version))
			goto unpack_error;
		if(tmp_ptr->ret_cnt != list_count(tmp_ptr->ret_list)) {
			error("unpack_agent_response: ret_list related error!");
		}
	}
	...
	return SLURM_SUCCESS;
unpack_error:
	slurm_free_response_ctld(tmp_ptr);
	*msg = NULL;
	return SLURM_ERROR;
}

static int
_pack_mediate_node_info_msg(mediate_node_info_msg_t *msg, Buf buffer,
		uint16_t protocol_version)
{
	pack16(msg->normal_mediate_cnt, buffer);
	if(msg->normal_mediate_cnt > 0)
		packstr(msg->normal_mediate_nodelist, buffer);
	pack16(msg->wrong_mediate_cnt, buffer);
	if(msg->wrong_mediate_cnt > 0)
		packstr(msg->wrong_mediate_nodelist, buffer);
	pack16(msg->busy_mediate_cnt, buffer);
	if(msg->busy_mediate_cnt > 0)
		packstr(msg->busy_mediate_nodelist, buffer);
	return SLURM_SUCCESS;
}


static int
_unpack_mediate_node_info_msg(mediate_node_info_msg_t **msg_ptr, Buf buffer,
		uint16_t protocol_version)
{
	uint32_t uint32_tmp = 0;
	mediate_node_info_msg_t *msg = xmalloc(sizeof(mediate_node_info_msg_t));
	*msg_ptr = msg;
	if (protocol_version >= SLURM_MIN_PROTOCOL_VERSION) {
		safe_unpack16(&msg->normal_mediate_cnt, buffer);
		
		if(msg->normal_mediate_cnt > 0)
			safe_unpackstr_xmalloc(&msg->normal_mediate_nodelist, &uint32_tmp, buffer);

		safe_unpack16(&msg->wrong_mediate_cnt, buffer);
		if(msg->wrong_mediate_cnt > 0)
			safe_unpackstr_xmalloc(&msg->wrong_mediate_nodelist, &uint32_tmp, buffer);

		safe_unpack16(&msg->busy_mediate_cnt, buffer);
		if(msg->busy_mediate_cnt > 0)
			safe_unpackstr_xmalloc(&msg->busy_mediate_nodelist, &uint32_tmp, buffer);
	}
	return SLURM_SUCCESS;
unpack_error:
	slurm_free_mediate_info(msg);
	*msg_ptr = NULL;
	return SLURM_ERROR;
}
...
pack_msg(slurm_msg_t const *msg, Buf buffer)
{
	...
	case RESPONSE_NODE_REGISTRATION:
		_pack_node_reg_resp(
			...
			buffer, msg->protocol_version);
		break;
	case REQUEST_PING_MEDIATE:
	case RESPONSE_PING_MEDIATE:
	case REQUEST_MEDIATE_INFO:	
	...
	case RESPONSE_UPDATE_CRONTAB:
		...
		break;
	case MEDIATE_RESPONSE_CTLD:
		_pack_mediate_response_msg((agent_response_t *) msg->data, buffer,
					msg->protocol_version);
		break;
	case RESPONSE_MEDIATE_INFO:
		_pack_mediate_node_info_msg((mediate_node_info_msg_t *) msg->data, buffer,
					msg->protocol_version);
		break;
	default:
...
unpack_msg(slurm_msg_t * msg, Buf buffer)
{
	...
	case RESPONSE_UPDATE_CRONTAB:
		...
		break;
	case MEDIATE_RESPONSE_CTLD:
		rc = _unpack_mediate_response_msg((agent_response_t **) &
			(msg->data), buffer,
			msg->protocol_version);
		break;
	case RESPONSE_MEDIATE_INFO:
		rc = _unpack_mediate_node_info_msg((mediate_node_info_msg_t **) &
			(msg->data), buffer,
			msg->protocol_version);
			break;
	default:
