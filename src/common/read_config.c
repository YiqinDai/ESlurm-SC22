...
static int _load_mediate_host(slurm_conf_t *conf); 
...
s_p_options_t slurm_conf_options[] = {
	...
	{"MediateWidth", S_P_UINT16},					
	{"MediateMachine", S_P_STRING},					
	{NULL}
};
...
{
	int i, j;
	conf->mediate_machine = xcalloc(100, sizeof(char *));
	conf->mediate_addr = xcalloc(100, sizeof(char *));
	conf->mediate_cnt = 0;
	char *tmp_mediate_machine = xmalloc(100);
	if (!s_p_get_string(&tmp_mediate_machine,"MediateMachine", conf_hashtbl))
		debug("No MediateHost defined.");
	else {
		hostlist_t mediate_hostlist = hostlist_create(tmp_mediate_machine);
		char* node_name = NULL;
		while ((node_name = hostlist_shift(mediate_hostlist))) {
			conf->mediate_machine[conf->mediate_cnt] = xstrdup(node_name);
			conf->mediate_cnt++;
			free(node_name);
		}
		hostlist_destroy(mediate_hostlist);
	}
	xfree(tmp_mediate_machine);
	for (i = 0; i < conf->mediate_cnt; i++) {
		if (!conf->mediate_addr[i]) {
			conf->mediate_addr[i] = xstrdup(conf->mediate_machine[i]);
		}
		for (j = 0; j < i; j++) {
			if (!xstrcmp(conf->mediate_machine[i],
				conf->mediate_machine[j])) {
				error("Duplicate SlurmctldHost records: %s",
					conf->mediate_machine[i]);
				goto error;
			}
		}
	}
...
error:
	if (conf->mediate_machine && conf->mediate_addr) {
		for (i = 0; i < conf->mediate_cnt; i++) {
			xfree(conf->mediate_machine[i]);
			xfree(conf->mediate_addr[i]);
		}
		xfree(conf->mediate_machine);
		xfree(conf->mediate_addr);
	}
	conf->mediate_cnt = 0;
	return SLURM_ERROR;
}
...
extern void free_slurm_conf(slurm_conf_t *ctl_conf_ptr, bool purge_node_hash)
{
	...
	for (i = 0; i < ctl_conf_ptr->mediate_cnt; i++) {
		xfree(ctl_conf_ptr->mediate_addr[i]);
		xfree(ctl_conf_ptr->mediate_machine[i]);
	}
	ctl_conf_ptr->mediate_cnt = 0;
	xfree (ctl_conf_ptr->mediate_addr);
	xfree (ctl_conf_ptr->mediate_machine);
	for (i = 0; i < ctl_conf_ptr->mediate_cnt; i++) {
		xfree(ctl_conf_ptr->mediate_addr[i]);
		xfree(ctl_conf_ptr->mediate_machine[i]);
	}
	...
	xfree (ctl_conf_ptr->mediate_addr);
	xfree (ctl_conf_ptr->mediate_machine);
	ctl_conf_ptr->mediate_width       		= NO_VAL16;	
                                      ...
                                      s_p_hashtbl_t *hashtbl)
{
	...
	if (s_p_get_uint16(&conf->mediate_width, "MediateWidth", hashtbl)){
		if (conf->mediate_width == 0){
			error("Mediate_width = 0 is invalid");
			conf->mediate_width = 2 * conf->tree_width;
		}
		if (conf->mediate_width <= conf->tree_width){
			error("Mediate_width < tree_width is invalid");
			conf->mediate_width = 2 * conf->tree_width;
		}
	} else{
		conf->mediate_width = 2 * conf->tree_width;
