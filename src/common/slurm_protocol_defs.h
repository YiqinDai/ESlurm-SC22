...
typedef enum {
	...
	MEDIATE_RESPONSE_CTLD,
	REQUEST_PING_MEDIATE,
	RESPONSE_PING_MEDIATE,
	REQUEST_MEDIATE_INFO,
	RESPONSE_MEDIATE_INFO,
} slurm_msg_type_t;
...
typedef struct slurm_protocol_agent {
	uint32_t	node_count;		/* number of nodes to communicate with */
	uint16_t	retry;			/* if set, keep trying */
	uint16_t    addr_flag;
	slurm_addr_t    *addr;      /* if set will send to this addr not hostlist */
	char*	nodelist;		/* hostlist containing the nodes we are sending to */
	uint16_t        protocol_version; /* protocol version to use */
} agent_t;

typedef struct slurm_response_agent {
	uint16_t comm_err_flag;
	char* comm_err_nodelist;

	uint16_t signal_tasks_complete;//REQUEST_SIGNAL_TASKS
	...
	uint16_t* type;
	char* no_resp_nodelist;

	uint16_t error_flag;
	char* failed_nodelist;
	char* dupid_nodelist;
	char* done_nodelist;
	char* error_nodelist;

	uint16_t ret_cnt;//get_reply
	uint16_t version;
	List ret_list;
