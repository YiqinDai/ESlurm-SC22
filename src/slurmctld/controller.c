...
static void ping_mediate_nodes (void); 
...
static void ping_mediate_nodes (void)
{
	...
	ping_agent_args->msg_type = REQUEST_PING_MEDIATE;
	...
	for(int ll=0;ll<slurm_conf.mediate_cnt;ll++)	
	{
		hostlist_push_host(ping_agent_args->hostlist, slurm_conf.mediate_machine[ll]);
	...
	else 
	{
		...
		debug("Spawning ping mediate agent for %s", host_str);
		xfree(host_str);
...
static void *_slurmctld_background(void *no_data)
{
	...
	static time_t last_ping_mediate_node_time = (time_t) 0; 
    ...
    if (slurm_conf.mediate_cnt > 0)
    	ping_mediate_nodes();
    debug3("firstly ping the mediate nodes");
	...
	if (!last_ping_mediate_node_time) {
			last_ping_mediate_node_time = now + (time_t)MIN_CHECKIN_TIME -
					      ping_mediate_interval / 2;  	     		
		}
    	...
    	if (slurm_conf.mediate_cnt > 0 && difftime(now, last_ping_mediate_node_time) >= ping_mediate_interval)  
    	{
    		now = time(NULL);
    		last_ping_mediate_node_time = now;
    		ping_mediate_nodes();
    	}
