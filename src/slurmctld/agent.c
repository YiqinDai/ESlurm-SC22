...
//static void get_predicted_failure(int node_count,hostlist_t h1, uint16_t* Failure);
static int mediate_back_process_per_thread(agent_response_t* agent_response, agent_arg_t* agent_arg_ptr);
static int send_and_recv_msgs_through_mediate(agent_arg_t* agent_arg_ptr, agent_response_t* agent_response);
static void _notify_slurmctld_nodes_per_thread(agent_info_t *agent_ptr,
		int no_resp_cnt, int retry_cnt);
static void _queue_agent_retry_per_thread(agent_info_t * agent_info_ptr, int count);
static void *_wdog_per_thread(void *args);
static int get_agent_span(int* agent_span, int node_count, slurm_msg_type_t msg_type);
static void response_init(slurm_msg_t *resp, slurm_msg_t *msg);
...
static pthread_mutex_t last_choice_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t mediate_state_init_mutex = PTHREAD_MUTEX_INITIALIZER;
static int* mediate_state = NULL;
...
extern void _slurm_rpc_dump_mediate_nodes(slurm_msg_t * msg)
{
    mediate_node_info_msg_t* resp = xmalloc(sizeof(mediate_node_info_msg_t));
    if (slurm_conf.mediate_cnt > 0)
    {
    	int i = 0;
    	hostlist_t normal_mediate_hostlist = hostlist_create(NULL);
    	hostlist_t wrong_mediate_hostlist = hostlist_create(NULL);
    	hostlist_t busy_mediate_hostlist = hostlist_create(NULL);
    	char *mediate_tmp = NULL;
    	for(i = 0; i < slurm_conf.mediate_cnt; i++)
    	{
    		mediate_tmp = slurm_conf.mediate_machine[i];
    		if(mediate_state == NULL)
    		{
    			slurm_mutex_lock(&mediate_state_init_mutex);
    			mediate_state = xcalloc(slurm_conf.mediate_cnt, sizeof(int));
    			for(int xx = 0; xx < slurm_conf.mediate_cnt; xx++)
    			{
    				mediate_state[xx] = 0;
    			}
    			slurm_mutex_unlock(&mediate_state_init_mutex);
    		}
    		if(mediate_state[i] == 2)
    			hostlist_push_host(normal_mediate_hostlist, mediate_tmp);
    		else if(mediate_state[i] == 0)
    			hostlist_push_host(wrong_mediate_hostlist, mediate_tmp);
    		else
    			hostlist_push_host(busy_mediate_hostlist, mediate_tmp);
    	}
    	hostlist_uniq(normal_mediate_hostlist);
    	hostlist_uniq(wrong_mediate_hostlist);
    	hostlist_uniq(busy_mediate_hostlist);
    	resp->normal_mediate_cnt = hostlist_count(normal_mediate_hostlist);
    	resp->normal_mediate_nodelist = hostlist_ranged_string_xmalloc(normal_mediate_hostlist);
    	resp->wrong_mediate_cnt = hostlist_count(wrong_mediate_hostlist);
    	resp->wrong_mediate_nodelist = hostlist_ranged_string_xmalloc(wrong_mediate_hostlist);
    	resp->busy_mediate_cnt = hostlist_count(busy_mediate_hostlist);
    	resp->busy_mediate_nodelist = hostlist_ranged_string_xmalloc(busy_mediate_hostlist);
    	hostlist_destroy(normal_mediate_hostlist);
    	hostlist_destroy(wrong_mediate_hostlist);
    	hostlist_destroy(busy_mediate_hostlist);
    }
    else
    {
    	resp->normal_mediate_cnt = 0;
    	resp->normal_mediate_nodelist = NULL;
    	resp->wrong_mediate_cnt = 0;
    	resp->wrong_mediate_nodelist = NULL;
    	resp->busy_mediate_cnt = 0;
    	resp->busy_mediate_nodelist = NULL;
    }
    
    slurm_msg_t resp_msg ;
    response_init(&resp_msg, msg);
    resp_msg.msg_type = RESPONSE_MEDIATE_INFO;
    resp_msg.data = resp;
    resp = NULL;
    /* send message */
    slurm_send_node_msg(msg->conn_fd, &resp_msg);
     slurm_free_mediate_info(resp_msg.data);
    return;
}
...
static void get_predicted_failure(int node_count,hostlist_t h1, uint16_t* Failure)
{
    ...
    int node_index = 0;
    hostlist_t htmp = hostlist_create(NULL);
    htmp = hostlist_copy(h1);
    while ((node_name = hostlist_shift(htmp)))
    {
    	now = time(NULL);
    	node_ptr = find_node_record(node_name);
    	if(!node_ptr)
    	{
    		error("we cannot find the purpose node %s when predicted its failure",node_name);
    		Failure[node_index] = 3;
    		break;
    	}
    	if (now - node_ptr->last_response > slurm_conf.slurmd_timeout)
    		Failure[node_index] = 3;
    	else if (now - node_ptr->last_response > slurm_conf.slurmd_timeout / 2)
    		Failure[node_index] = 2;
    	else if (now - node_ptr->last_response > slurm_conf.slurmd_timeout / 3)
    		Failure[node_index] = 1;
    	else
    		Failure[node_index] = 0;
    	node_index++;
		free(node_name);
    }
    hostlist_destroy(htmp);
    return;
}
*/
...
static int get_agent_span(int* agent_span, int node_count, slurm_msg_type_t msg_type)
{
    ...
    for(int yy = 0; yy < slurm_conf.mediate_cnt; yy++)
    {
    	if(mediate_state[yy] == 2)
    		normal_mediate_cnt++;
    }
    if(node_count <= slurm_conf.mediate_width ||
       normal_mediate_cnt == 0  ||
       msg_type == SRUN_PING ||
    ...
    {
    	agent_span[0] = node_count;
    	return 1;
    }
    else if(node_count <= normal_mediate_cnt * slurm_conf.mediate_width)
    {
    	if(node_count % slurm_conf.mediate_width == 0)
    	{
    		count = node_count/slurm_conf.mediate_width;
    		for(int i = 0; i < count; i++)
    		{
    			agent_span[i] = slurm_conf.mediate_width;
    		}
    	}
    	else
    	{
    		count = node_count/slurm_conf.mediate_width + 1;
    		for(int i = 0; i < count - 1 ; i++)
    		{
    			agent_span[i] = slurm_conf.mediate_width;
    		}
    		agent_span[count - 1] = node_count % slurm_conf.mediate_width;
    	}
    	return count;
    }
    else
    {
    	/*if(msg_type == REQUEST_NODE_REGISTRATION_STATUS && node_count > 60000)
    	{
    		count = normal_mediate_cnt * 2;
    		for(int i = 0; i < 2 * normal_mediate_cnt; i++)
    		{
    			agent_span[i] = node_count / (2 * normal_mediate_cnt);
    		}
    		for(int i = 0; i < node_count % (2 * normal_mediate_cnt); i++)
    		{
    			agent_span[i]++;
    		}
    	}
    	...
    	{*/
    		count = normal_mediate_cnt;
    		for(int i = 0; i < normal_mediate_cnt; i++)
    		{
    			agent_span[i] = node_count / normal_mediate_cnt;
    		}
    		for(int i = 0; i < node_count % normal_mediate_cnt; i++)
    		{
    			agent_span[i]++;
    		}
    	//}
...
static int send_and_recv_msgs_through_mediate(agent_arg_t* agent_arg_ptr, agent_response_t* agent_response)
{
    ...
    agent_t *agent_sending = xmalloc(sizeof(agent_t)); 
    agent_t_init(agent_sending);
    agent_sending->node_count = agent_arg_ptr->node_count;
    agent_sending->retry = agent_arg_ptr->retry;
    agent_sending->addr = agent_arg_ptr->addr;
    if (agent_sending->addr)
    {
    	agent_sending->addr_flag = 1;
    }
    else
    {
    	agent_sending->addr_flag = 0;
    }
    agent_sending->nodelist = hostlist_ranged_string_xmalloc(agent_arg_ptr->hostlist);
    agent_sending->protocol_version = agent_arg_ptr->protocol_version;
    
    req_msg.msg_type = agent_arg_ptr->msg_type;
    req_msg.data = agent_arg_ptr->msg_args;

    uint16_t m = slurm_conf.mediate_width;
    uint16_t n = slurm_conf.tree_width;
    int cnt = agent_arg_ptr -> node_count;
    int mediate_timeout = 0;
    	...
    	(agent_arg_ptr->msg_type != SRUN_JOB_COMPLETE)	) 
    {
    	if (agent_arg_ptr->addr)
    		mediate_timeout = slurm_conf.msg_timeout * 1000;
    	else
    		mediate_timeout = ((2 * (cnt / m + 1) / n + 1) + 5) * slurm_conf.msg_timeout * 1000;
    }
    else
    {
    	mediate_timeout = 2 * slurm_conf.msg_timeout * 1000;
    }
    
    int newchoice = 0;
    int total_retry_cnt = -1;
    int good_retry_cnt = 0;
    int rc = -1;
    
find_next_mediate:
    total_retry_cnt++;
    if (total_retry_cnt >= slurm_conf.mediate_cnt + 1 || total_retry_cnt - good_retry_cnt >= 2)
    {
    	req_msg.data = NULL;
    	resp_msg.data = NULL;
    	if(agent_sending->nodelist)
    		xfree(agent_sending->nodelist);
    	if(agent_sending->addr)
    		agent_sending->addr = NULL;
    	if(agent_sending)
    		xfree(agent_sending);
    	return SLURM_ERROR;
    }
    	
    slurm_mutex_lock(&last_choice_mutex);	
    if (last_choice + 1 >= slurm_conf.mediate_cnt)
    	last_choice = -1;
    newchoice = ++last_choice;
    slurm_mutex_unlock(&last_choice_mutex);	
    if(mediate_state == NULL)
    {
    	slurm_mutex_lock(&mediate_state_init_mutex);
    	mediate_state = xcalloc(slurm_conf.mediate_cnt, sizeof(int));
    ...
    if(!mediate_state[newchoice])
    {
    	good_retry_cnt++;
    	goto find_next_mediate;
    }
    debug("The mediate node we choose is %s, and the node count of this mediate node is %d", slurm_conf.mediate_addr[newchoice], agent_arg_ptr->node_count);
    char* thisaddr = slurm_conf.mediate_addr[newchoice];
    slurm_set_addr(&req_msg.address, slurm_conf.slurmd_port, thisaddr);
	thisaddr = NULL;

    rc = slurm_mediate_send_recv_node_msg(&req_msg, &resp_msg, agent_sending, mediate_timeout);
    if(rc == -3)
    {
    	error("open the connection of mediate node %s failed",slurm_conf.mediate_addr[newchoice]);
    	mediate_state[newchoice] = 0;
    	goto find_next_mediate;
    }
    else if (rc == -2)
    {
    	error("send to mediate node %s failed",slurm_conf.mediate_addr[newchoice]);
    	mediate_state[newchoice] = 0;
    	goto find_next_mediate;
    }
    else if(rc == 0)
    {
    	if(resp_msg.msg_type != MEDIATE_RESPONSE_CTLD)
    	{	
    		error("we receive msg from mediate node %s but it is not what we are looking for",slurm_conf.mediate_addr[newchoice]);
    		mediate_state[newchoice] = 0;
    		goto find_next_mediate;
    	}
    	mediate_state[newchoice] = 2;
    	*agent_response = *((agent_response_t*)(resp_msg.data));
    	debug3("we successfully get our response from mediate node %s",slurm_conf.mediate_addr[newchoice]);
    	req_msg.data = NULL;
    	resp_msg.data = NULL;
    	if(agent_sending->nodelist)
    		xfree(agent_sending->nodelist);
    ...
    {
    	error("receive from mediate node %s failed and the rc form slurm_mediate_send_recv_node_msg = %d", slurm_conf.mediate_addr[newchoice], rc);
    	mediate_state[newchoice] = 0;
    	goto find_next_mediate;
    }
...
static int mediate_back_process_per_thread(agent_response_t* agent_response, agent_arg_t* agent_arg_ptr)
{
    ...
    slurm_msg_type_t msg_type = agent_arg_ptr->msg_type;
    ...
    char* name = NULL;
    if (agent_response->comm_err_nodelist && agent_response->comm_err_flag)
    {
    	if (agent_response->ret_cnt != 0)
    		error("wrong comm_err_nodelist!");
    	else
    	{
    		hostlist_t comm_err_hostlist = hostlist_create(agent_response->comm_err_nodelist);
    		lock_slurmctld(node_read_lock);
    		while ((name = hostlist_shift(comm_err_hostlist)))
    		{
    			_comm_err(name, msg_type);
				free(name);
    		}
    ...
    if ((msg_type != REQUEST_JOB_NOTIFY) &&
    	...
    	(msg_type != SRUN_STEP_SIGNAL) &&
    	(msg_type != SRUN_JOB_COMPLETE)&&
    	agent_response->ret_cnt !=0 &&
    	agent_response->ret_list)
    {
    	debug3("the count of ret_list = %d",list_count(agent_response->ret_list));
    	ListIterator itr;
    	hostlist_t retry_hostlist = hostlist_create(NULL);
    	hostlist_t no_resp_hostlist = hostlist_create(NULL);
    	hostlist_t done_hostlist = hostlist_create(NULL);
    	if (agent_response->retry_flag > 0)
    	{
    		retry_hostlist = hostlist_create(agent_response->retry_nodelist);
    	}
    	if (agent_response->no_resp_cnt > 0)
    	{
    		no_resp_hostlist = hostlist_create(agent_response->no_resp_nodelist);
    	}
    	if (agent_response->done_nodelist)
    	{
    		done_hostlist = hostlist_create(agent_response->done_nodelist);
    	}
    	itr = list_iterator_create(agent_response->ret_list);
    	while ((ret_data_info = list_next(itr)))
    	{
    		...
    		if (ret_data_info->type == RESPONSE_PING_SLURMD)
    		{
    			...
    			ping_resp = (ping_slurmd_resp_msg_t *)ret_data_info->data;
    			lock_slurmctld(node_write_lock);
    			reset_node_load(ret_data_info->node_name, ping_resp->cpu_load);
    			reset_node_free_mem(ret_data_info->node_name, ping_resp->free_mem);
    		...
    		if (is_kill_msg && (rc == ESLURMD_KILL_JOB_ALREADY_COMPLETE))
    		{
    			...
    			if (job_epilog_complete(kill_job->step_id.job_id, ret_data_info->node_name, rc))
    				run_scheduler = true;
    		...
    		{
    			lock_slurmctld(node_write_lock);
    			update_node_record_acct_gather_data(ret_data_info->data);
    			unlock_slurmctld(node_write_lock);
    		}
    		...
    		{
    			batch_job_launch_msg_t *launch_msg_ptr = agent_arg_ptr->msg_args;
    			job_id = launch_msg_ptr->job_id;
    			info("Killing non-startable batch JobId=%u: %s", job_id, slurm_strerror(rc));
    			lock_slurmctld(job_write_lock);
    			job_complete(job_id, slurm_conf.slurm_user_id, false, false, _wif_status());
    			unlock_slurmctld(job_write_lock);
    			continue;
    		}
    		...
    		{
    			resource_allocation_response_msg_t *msg_ptr = agent_arg_ptr->msg_args;
    			job_id = msg_ptr->job_id;
    			info("Killing interactive JobId=%u: %s", job_id, slurm_strerror(rc));
    			lock_slurmctld(job_write_lock);
    			job_complete(job_id, slurm_conf.slurm_user_id, false, false, _wif_status());
    			unlock_slurmctld(job_write_lock);
    			continue;
    			...
    			(rc == SLURM_COMMUNICATIONS_CONNECTION_ERROR))
    		{
    			List pack_alloc_list = agent_arg_ptr->msg_args;
    		...
    		if (msg_type == REQUEST_SIGNAL_TASKS)
    		{
    			...
    			signal_tasks_msg_t *msg_ptr = agent_arg_ptr->msg_args;
    		...
    		default:
    			...
    			if (!srun_agent && rc && ret_data_info->type != RESPONSE_FORWARD_FAILED)
    			{
    				debug("special case: msg_type = %s, rc_from_comm_err = %d (!= 0), ret_data_type = %d", rpc_num2string(msg_type), rc, ret_data_info->type);
    				info("you are suppose to reboot %s's slurmd", ret_data_info->node_name);
    				last_flag = 1;
    				if (agent_response->no_resp_flag == 0 && msg_type == REQUEST_BATCH_JOB_LAUNCH)
    					agent_response->no_resp_flag = 1;
    				if (agent_response->retry_flag == 0 && agent_arg_ptr->retry)
    				{
    					struct node_record *node_ptr;
    					node_ptr = find_node_record(ret_data_info->node_name);
    					if(!IS_NODE_DOWN(node_ptr) && !IS_NODE_POWER_SAVE(node_ptr))
    					{
    						agent_response->retry_flag = 1;
    						retry_hostlist = hostlist_create(ret_data_info->node_name);
    						debug("special case: we add the %s in the retry_nodelist",ret_data_info->node_name);
    					}
    				}
    				else
    				{
    					...
    					if(!IS_NODE_DOWN(node_ptr) && !IS_NODE_POWER_SAVE(node_ptr))
    					{
    						hostlist_push_host(retry_hostlist, ret_data_info->node_name);
    						debug("special case: we add the %s in the retry_nodelist",ret_data_info->node_name);
    					}
    				}
    				node_not_resp(ret_data_info->node_name, agent_response->start_time, ret_data_info->type);
    				debug("special case: we add the %s in the no_resp_nodelist",ret_data_info->node_name);
    				if (agent_response->done_nodelist)
    				{
    					hostlist_delete_host(done_hostlist, ret_data_info->node_name);
    					debug("special case: we delete the %s in the done_nodelist",ret_data_info->node_name);
    				}
    			}
    		}
    	}
    	if (msg_type == RESPONSE_HET_JOB_ALLOCATION && last_flag == 1)
    	{
    		if (agent_response->job_id != NO_VAL)
    		{
    			agent_response->job_id = agent_response->step_id = NO_VAL;
    			debug("special case: we delete the job_id and the step_id");
    		}
    	}
    	if(hostlist_count(retry_hostlist) > 0)
    		agent_response->retry_nodelist = hostlist_ranged_string_xmalloc(retry_hostlist);
    	if(hostlist_count(no_resp_hostlist) > 0)
    		agent_response->no_resp_nodelist = hostlist_ranged_string_xmalloc(no_resp_hostlist);
    	if(hostlist_count(done_hostlist) > 0)	
    		agent_response->done_nodelist = hostlist_ranged_string_xmalloc(done_hostlist);
    	hostlist_destroy(retry_hostlist);
    	hostlist_destroy(no_resp_hostlist);
    	hostlist_destroy(done_hostlist);
    	list_iterator_destroy(itr);	
    }
    if (agent_response->signal_tasks_complete > 0) 
    {
    
    	if (agent_response->ret_list || msg_type != REQUEST_SIGNAL_TASKS)
    	{
    		error("comm_err_nodelist error for appearing in srun_agent");
    		return SLURM_ERROR;
    	}
    	else
    	{
    		...
    		if ((msg_ptr->signal != SIGCONT) &&
    			(msg_ptr->signal != SIGSTOP))
    		{
    			error("comm_err_nodelist error for appearing in wrong signal");
    		}
    		else
    		{
    			job_id = msg_ptr->step_id.job_id;
    			...
    			if (job_ptr)
    				job_ptr->job_state &= ~JOB_SIGNALING;
    			unlock_slurmctld(job_write_lock);
    			debug("successfully deal with the signal_tasks_complete");
    		}
    	}
    }
    
    if (agent_response->no_resp_flag > 0)
    {
    	if (msg_type == REQUEST_BATCH_JOB_LAUNCH)
    	{
    		/* Requeue the request */
    		...
    		job_complete(job_id, slurm_conf.slurm_user_id,
    			true, false, 0);
    		unlock_slurmctld(job_write_lock);
    		debug("successfully deal with the no_resp_flag");
    	}
    	else
    	{
    		error("wrong no_resp_flag*");
    	}
    }
    name = NULL;
    if (agent_response->retry_flag > 0)
    {
    	if (agent_arg_ptr->retry != 1)
    	{
    		error("retry wrong for the reason that the msg do not need retry");
    	}
    	else
    	{
    		...
    		struct node_record *node_ptr;
#endif
    		agent_arg_t *agent_arg_ptr_new = NULL;
    		queued_request_t *queued_req_ptr_new = NULL;
    		agent_arg_ptr_new = xmalloc(sizeof(agent_arg_t));
    		agent_arg_ptr_new->hostlist = hostlist_create(NULL);
    		agent_arg_ptr_new->retry = 1;
    		agent_arg_ptr_new->msg_type = msg_type;
    		
    		if(agent_arg_ptr->msg_type == REQUEST_SIGNAL_TASKS)
    		{
    			agent_arg_ptr_new->msg_args = xmalloc(sizeof(signal_tasks_msg_t));
    			*((signal_tasks_msg_t*)(agent_arg_ptr_new->msg_args)) = *( (signal_tasks_msg_t*) (agent_arg_ptr->msg_args) );
    		}
    		else if(agent_arg_ptr->msg_type == REQUEST_KILL_TIMELIMIT)
    		{
    			agent_arg_ptr_new->msg_args = xmalloc(sizeof(kill_job_msg_t));
    			*((kill_job_msg_t*)(agent_arg_ptr_new->msg_args)) = *( (kill_job_msg_t*) (agent_arg_ptr->msg_args));
    		}
    		...
    		else if(agent_arg_ptr->msg_type == REQUEST_TERMINATE_TASKS)	
    		{
    			agent_arg_ptr_new->msg_args = xmalloc(sizeof(step_complete_msg_t));
    			*((step_complete_msg_t*)(agent_arg_ptr_new->msg_args))= *( (step_complete_msg_t*) (agent_arg_ptr->msg_args));
    		}
    		else
    		{
    			error("error msg_type for retry(type = %s)",rpc_num2string(agent_arg_ptr->msg_type));
    			hostlist_destroy(agent_arg_ptr_new->hostlist);
    			xfree(agent_arg_ptr_new);
    			goto no_retry;
    		}
    		
    		int j = 0;
    		hostlist_t retry_hostlist =  hostlist_create(agent_response->retry_nodelist);
    		while ((name = hostlist_shift(retry_hostlist)))
    		{
#ifdef HAVE_FRONT_END
...
#endif
    			if (node_ptr &&((IS_NODE_DOWN(node_ptr) || IS_NODE_POWER_SAVE(node_ptr))))
    			{
    				continue;
    			}
    			hostlist_push_host(agent_arg_ptr_new->hostlist,name);
				free(name);
    			j++;
    		}
    		agent_arg_ptr_new->node_count = hostlist_count(agent_arg_ptr_new->hostlist);
    		
    		if (agent_arg_ptr_new->node_count == 0)
    		{
    			debug("Queue RPC msg_type=%u, no nodes for retry",
    				agent_arg_ptr_new->msg_type);
    			hostlist_destroy(agent_arg_ptr_new->hostlist);
    			xfree(agent_arg_ptr_new);
    		}
    		else
    		{
    			debug("Queue RPC msg_type=%u, nodes=%d for retry,agent_arg_ptr_new->hostlist = %s ",
    				agent_arg_ptr_new->msg_type, j,hostlist_ranged_string_xmalloc(agent_arg_ptr_new->hostlist));
    			queued_req_ptr_new = xmalloc(sizeof(queued_request_t));
    			queued_req_ptr_new->agent_arg_ptr = agent_arg_ptr_new;
    			queued_req_ptr_new->last_attempt = time(NULL);
    			slurm_mutex_lock(&retry_mutex);
    			if (retry_list == NULL)
    				retry_list = list_create(_list_delete_retry);
    			(void)list_append(retry_list, (void *)queued_req_ptr_new);
...
no_retry:	
    lock_slurmctld(node_write_lock);
    char *down_msg;
    if (agent_response->no_resp_nodelist && agent_response->no_resp_cnt > 0)
    {
    	debug3("no_resp_nodelist in the response msg: %s",agent_response->no_resp_nodelist);
    	int msg_index = 0;
    	time_t start_time = 0;
    	slurm_msg_type_t no_resp_type;
    	
    	hostlist_t no_resp_hostlist = hostlist_create(agent_response->no_resp_nodelist);
    	while ((name = hostlist_shift(no_resp_hostlist)))
    	{
    		start_time = time(NULL);
    		no_resp_type = agent_response->type[msg_index];
    		msg_index++;
    		node_not_resp(name, start_time, no_resp_type);
			free(name);
    	}
    }
    if (agent_response->failed_nodelist)
    {
    	debug("failed_nodelist in the response msg*: %s",agent_response->failed_nodelist);
    	hostlist_t failed_hostlist = hostlist_create(agent_response->failed_nodelist);
    	while ((name = hostlist_shift(failed_hostlist)))
    	{
    		drain_nodes(name,
    ...
    if (agent_response->dupid_nodelist)
    {
    	debug("dupid_nodelist in the response msg*: %s", agent_response->dupid_nodelist);
    	hostlist_t dupid_hostlist = hostlist_create(agent_response->dupid_nodelist);
    	while ((name = hostlist_shift(dupid_hostlist)))
    	{
    		drain_nodes(name,
    			"Duplicate jobid",
    			slurm_conf.slurm_user_id);
    ...
    if (agent_response->done_nodelist)
    {
    	debug3("done_nodelist in the response msg: %s",agent_response->done_nodelist);
    	hostlist_t done_hostlist = hostlist_create(agent_response->done_nodelist);
    	while ((name = hostlist_shift(done_hostlist)))
    	{
    		node_did_resp(name);
			free(name);
    	}
...
static void* _agent_per_group(void* args)
{
    int i, delay;
    ...
    agent_response_t *agent_response =  xmalloc(sizeof(agent_response_t));
    agent_response_t_init(agent_response);
    debug3("one of threads: msg_type = %s", rpc_num2string(agent_arg_ptr->msg_type));
    if(hostlist_count(agent_arg_ptr->hostlist) > 0)
    	debug3("one of threads: hostlist = %s ",hostlist_ranged_string_xmalloc(agent_arg_ptr->hostlist));
    if(agent_arg_ptr->addr)
    	debug3("one of threads: we have the addr");
    if (send_and_recv_msgs_through_mediate(agent_arg_ptr, agent_response) == SLURM_SUCCESS)
    {
    	mediate_back_process_per_thread(agent_response, agent_arg_ptr);
    	if(agent_response)
    	{
    		if(agent_response->comm_err_nodelist)
    			xfree(agent_response->comm_err_nodelist);
    		if(agent_response->retry_nodelist)
    			xfree(agent_response->retry_nodelist);
    		if(agent_response->no_resp_start_time)
    			xfree(agent_response->no_resp_start_time);
    		if(agent_response->type)	
    			xfree(agent_response->type);
    		if(agent_response->no_resp_nodelist)
    			xfree(agent_response->no_resp_nodelist);
    		if(agent_response->failed_nodelist)
    			xfree(agent_response->failed_nodelist);
    		if(agent_response->dupid_nodelist)
    			xfree(agent_response->dupid_nodelist);
    		if(agent_response->done_nodelist)
    			xfree(agent_response->done_nodelist);
    		if(agent_response->error_nodelist)
    			xfree(agent_response->error_nodelist); 
    		if(agent_response->ret_list)
    			FREE_NULL_LIST(agent_response->ret_list);
    		xfree(agent_response);
    	}
    	goto mediate_process;
    }
    else
    {
    	debug("one of threads: failed");
    }
    
    debug("one of threads: start the normal process");
    if(hostlist_count(agent_arg_ptr->hostlist) > 0)
    	debug("one of threads: normal start hostlist = %s", hostlist_ranged_string_xmalloc(agent_arg_ptr->hostlist));
    if(agent_arg_ptr->addr)
    	debug("one of threads: normal start have the addr");

    agent_info_ptr = _make_agent_info(agent_arg_ptr);
    thread_ptr = agent_info_ptr->thread_struct;
    /* start the watchdog thread */
    slurm_thread_create(&thread_wdog, _wdog_per_thread, agent_info_ptr);
    if (slurm_conf.debug_flags & DEBUG_FLAG_AGENT) {
    	info("%s: one group New agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
    ...
    debug3("%s: one group New agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
    	     __func__, agent_info_ptr->thread_count,
    	     agent_info_ptr->threads_active,
    	     agent_info_ptr->retry ? 'T' : 'F',
    	     agent_info_ptr->get_reply ? 'T' : 'F',
    ...
    if (delay > (slurm_conf.msg_timeout * 2)) {
    	debug("one group of the agent msg_type=%u ran for %d seconds",
    ...
    if (slurm_conf.debug_flags & DEBUG_FLAG_AGENT) {
    	info("%s: one group for end agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
    	     __func__, agent_info_ptr->thread_count,
    	     agent_info_ptr->threads_active,
    	     agent_info_ptr->retry ? 'T' : 'F',
    	     agent_info_ptr->get_reply ? 'T' : 'F',
    ...
    debug3("one group for agent : %s: end agent thread_count:%d threads_active:%d retry:%c get_reply:%c msg_type:%s protocol_version:%hu",
    	     __func__, agent_info_ptr->thread_count,
    	     agent_info_ptr->threads_active,
    	     agent_info_ptr->retry ? 'T' : 'F',
    	     agent_info_ptr->get_reply ? 'T' : 'F',
...
mediate_process:
    delay = (int)difftime(time(NULL), begin_time);
    if (delay > (slurm_conf.msg_timeout * 2)) {
    	debug("one group for agent msg_type=%u ran for %d seconds",
    		agent_arg_ptr->msg_type, delay);
    }
    debug3("one group for agent: agent msg_type=%u ran for %d seconds",
    		agent_arg_ptr->msg_type, delay);
    		
cleanup:
    agent_arg_ptr->msg_args = NULL;
    agent_arg_ptr->addr = NULL;
...
void *agent(void *args)
{
    ...
    if( slurm_conf.mediate_cnt == 0 || (agent_arg_ptr->msg_type != REQUEST_PING && 
    					 agent_arg_ptr->msg_type != REQUEST_NODE_REGISTRATION_STATUS &&
    					 agent_arg_ptr->msg_type != REQUEST_TERMINATE_JOB ))
    	goto normal_start;
    int* agent_span;
    agent_span = xcalloc(slurm_conf.mediate_cnt * 2,sizeof(int));
    int agent_count = 0;	
    agent_count = get_agent_span(agent_span, agent_arg_ptr->node_count, agent_arg_ptr->node_count);
    debug("msg_type = %s, we use %d mediate nodes", rpc_num2string(agent_arg_ptr->msg_type), agent_count);	
    char *node_name_in_agent = NULL;
    pthread_t * agent_per_group_pthread = xcalloc(agent_count,sizeof(pthread_t));
    int agent_index = 0;
    
    for(agent_index = 0; agent_index < agent_count; agent_index++)
    {
    	agent_per_group_pthread[agent_index] = agent_index+1;
    	node_name_in_agent = NULL;
    	agent_arg_t *agent_arg_new_ptr;
    	agent_arg_new_ptr = xmalloc(sizeof(agent_arg_t));
    	
    	agent_arg_new_ptr->hostlist = hostlist_create(NULL);
    	agent_arg_new_ptr->retry = agent_arg_ptr->retry;
    	agent_arg_new_ptr->addr = agent_arg_ptr->addr;//no seperate
    	agent_arg_new_ptr->msg_args = agent_arg_ptr->msg_args;//no seperate (do not forget set it to NULL)
    	agent_arg_new_ptr->msg_type = agent_arg_ptr->msg_type;
    	agent_arg_new_ptr->protocol_version = agent_arg_ptr->protocol_version;
    	debug3("msg_type = %s, the number of host in thread[%d]= %d", rpc_num2string(agent_arg_ptr->msg_type), agent_index,agent_span[agent_index]);
    	while( agent_span[agent_index] > 0)
    	{
    		node_name_in_agent = hostlist_shift(agent_arg_ptr->hostlist);
    		if(!node_name_in_agent)
    		{
    			error("agent_group_span wrong!");
    			break;
    		}
    		hostlist_push_host(agent_arg_new_ptr->hostlist, node_name_in_agent);
    		agent_span[agent_index]--;
		free(node_name_in_agent);
    	}
    	debug3("msg_type = %s, agent group NO.%d's hostlist = %s",rpc_num2string(agent_arg_ptr->msg_type), agent_index,hostlist_ranged_string_xmalloc(agent_arg_new_ptr->hostlist));
    	agent_arg_new_ptr->node_count = hostlist_count(agent_arg_new_ptr->hostlist);
    	//if(agent_arg_ptr->msg_type == REQUEST_NODE_REGISTRATION_STATUS && agent_arg_ptr->node_count > 60000)
    	// {
    	// 	if(agent_index <= agent_count / 2)
    	// 		slurm_thread_create(&agent_per_group_pthread[agent_index], _agent_per_group, agent_arg_new_ptr);
    	// 	else
    	// 	{
    	// 		usleep(3000000);
    	// 		slurm_thread_create(&agent_per_group_pthread[agent_index], _agent_per_group, agent_arg_new_ptr);
    	...
    	// }
    	slurm_thread_create(&agent_per_group_pthread[agent_index], _agent_per_group, agent_arg_new_ptr);
    	agent_arg_new_ptr = NULL;
    	node_name_in_agent = NULL;
    }
    for(agent_index = 0; agent_index < agent_count; agent_index++)
    {
    	pthread_join(agent_per_group_pthread[agent_index],NULL);
    	debug("successfully finish the thread %d",agent_index);
    }
    xfree(agent_span);
    goto mediate_process;
    
normal_start:
	debug("start the normal process");
	//info("agent: msg_type = %s", rpc_num2string(agent_arg_ptr->msg_type));
	if(hostlist_count(agent_arg_ptr->hostlist) > 0)
		debug("hostlist = %s", hostlist_ranged_string_xmalloc(agent_arg_ptr->hostlist));
    ...
    debug("agent_msg_type=%s ran for %d seconds",
...
cleanup:
	...
	if ((agent_arg_ptr->msg_type == REQUEST_PING) ||
	    (agent_arg_ptr->msg_type == REQUEST_HEALTH_CHECK) ||
	    (agent_arg_ptr->msg_type == REQUEST_ACCT_GATHER_UPDATE) ||
	    (agent_arg_ptr->msg_type == REQUEST_NODE_REGISTRATION_STATUS))
			ping_end();
	...
	if ((agent_thread_cnt + AGENT_THREAD_COUNT/2 + 2) < MAX_SERVER_THREADS)
...
static agent_info_t *_make_agent_info(agent_arg_t *agent_arg_ptr)
{
	...
	if ((agent_arg_ptr->msg_type != REQUEST_JOB_NOTIFY)	&&
	    ...
	    (agent_arg_ptr->msg_type != SRUN_STEP_SIGNAL)	&&
	    (agent_arg_ptr->msg_type != SRUN_JOB_COMPLETE)    &&
	    (agent_arg_ptr->msg_type != REQUEST_PING_MEDIATE)) {  
#ifdef HAVE_FRONT_END
...
static void *_wdog_per_thread(void *args)
{
	...
	if(agent_ptr->msg_type != REQUEST_PING_MEDIATE)
	{
		if (srun_agent) {
			_notify_slurmctld_jobs(agent_ptr);
		} else {
			_notify_slurmctld_nodes_per_thread(agent_ptr,
						thd_comp.no_resp_cnt,
						thd_comp.retry_cnt);
		}
	}
				    ...
				    int no_resp_cnt, int retry_cnt)
{
	...
	if (retry_cnt && agent_ptr->retry)
		_queue_agent_retry_per_thread(agent_ptr, retry_cnt);
...
static void *_thread_per_group_rpc(void *args)
{
	...
	if (task_ptr->get_reply) {
		...
		if (thread_ptr->addr) {
			...
			if( msg_type == REQUEST_PING_MEDIATE) {
				for (i = 0; i < slurm_conf.mediate_cnt; i++) {
					if (slurm_conf.mediate_machine[i] && slurm_conf.mediate_addr[i] && (!xstrcmp(thread_ptr->nodelist, slurm_conf.mediate_machine[i])))
						break;
				}
				if (i < slurm_conf.mediate_cnt) {
					char* thisaddr = slurm_conf.mediate_addr[i];
					slurm_set_addr(&msg.address, slurm_conf.slurmd_port, thisaddr);
					thisaddr = NULL;
				} else {
					goto cleanup;
				}
			} else if (slurm_conf_get_addr(thread_ptr->nodelist,
		...
		else if(msg_type == REQUEST_PING_MEDIATE) {
			slurm_msg_t *resp_msg =  xmalloc(sizeof(slurm_msg_t));
			slurm_msg_t_init(resp_msg);
			if(slurm_send_recv_node_msg(&msg,resp_msg,30000) == SLURM_SUCCESS && resp_msg->msg_type== RESPONSE_PING_MEDIATE) {
				hostlist_t mediate_host = hostlist_create(thread_ptr->nodelist);
				if(hostlist_count(mediate_host) == 1) {
					if (i < slurm_conf.mediate_cnt) {
						int j = 0;
						if(mediate_state == NULL) {
							slurm_mutex_lock(&mediate_state_init_mutex);
							mediate_state = xcalloc(slurm_conf.mediate_cnt, sizeof(int));
							for(j = 0; j < slurm_conf.mediate_cnt; j++) 
								mediate_state[j] = 0; 
							slurm_mutex_unlock(&mediate_state_init_mutex);
						}
						mediate_state[i] = 2;
					} else {
						error("mediate node %s cannot find", thread_ptr->nodelist);
					}
				}
				hostlist_destroy(mediate_host);
			} else {
				debug("failed receive the ping message from mediate node %s", thread_ptr->nodelist);
				hostlist_t mediate_host = hostlist_create(thread_ptr->nodelist);
...
static void _queue_agent_retry_per_thread(agent_info_t * agent_info_ptr, int count)
{
	...
	if(agent_arg_ptr->msg_type == REQUEST_SIGNAL_TASKS)
	{
		agent_arg_ptr->msg_args = xmalloc(sizeof(signal_tasks_msg_t));
		*((signal_tasks_msg_t*)agent_arg_ptr->msg_args) = **( (signal_tasks_msg_t**) (agent_info_ptr->msg_args_pptr));
	}
	else if(agent_arg_ptr->msg_type == REQUEST_KILL_TIMELIMIT)
	{
		agent_arg_ptr->msg_args = xmalloc(sizeof(kill_job_msg_t));
		*((kill_job_msg_t*)agent_arg_ptr->msg_args) = **( (kill_job_msg_t**) (agent_info_ptr->msg_args_pptr));
	}
	else if(agent_arg_ptr->msg_type == REQUEST_ABORT_JOB)
	{
		agent_arg_ptr->msg_args = xmalloc(sizeof(kill_job_msg_t));
	...
	{
		agent_arg_ptr->msg_args = xmalloc(sizeof(step_complete_msg_t));
		*((step_complete_msg_t*)agent_arg_ptr->msg_args) = **( (step_complete_msg_t**) (agent_info_ptr->msg_args_pptr));
	}
	else
	{
		error("error msg_type for retry(type = %s)",rpc_num2string(agent_arg_ptr->msg_type));
	...
	debug("Queue RPC msg_type=%u, node_count=%d , nodes=%s for retry",
	       agent_arg_ptr->msg_type, j, hostlist_ranged_string_xmalloc(agent_arg_ptr->hostlist));
...
static void _purge_agent_args(agent_arg_t *agent_arg_ptr)
{
	...
	if (agent_arg_ptr->msg_args) {
		...
		{
			if(agent_arg_ptr->msg_args != NULL)
